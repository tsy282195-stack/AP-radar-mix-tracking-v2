...
if strcmp(cfg.processing_framework, 'joint_2d3d')
    [est, joint_events] = run_filter_joint_legacy_backbone( ...
        ...
        fused_ids, event_meta, cfg, event_meta);
...
if ~isfield(cfg, 'metrics_enabled') || cfg.metrics_enabled
    ...
    try
        if strcmp(cfg.processing_framework, 'joint_2d3d')
            metrics_info = evaluate_joint_tracking_metrics(est, joint_events, cfg);
        else
...
if strcmp(cfg.processing_framework, 'joint_2d3d') && ...
    ...
    try
        assignment_export_info = export_measurement_track_assignments( ...
            est, joint_events, cfg);
        est.measurement_assignment_export_info = assignment_export_info;
    catch ME
        warning('量测-航迹对应Excel导出失败: %s', ME.message);
    end
end

%% ── 第6.2步：已提交的二维/三维升降维事务表 ───────────────────────────
if strcmp(cfg.processing_framework, 'joint_2d3d') && ...
        cfg.dimension_transition_export_enabled
    try
        transition_export_info = export_track_dimension_transitions(est, cfg);
        est.dimension_transition_export_info = transition_export_info;
    catch ME
        warning('二维/三维升降维事务Excel导出失败: %s', ME.message);
    end
end
...
if cfg.do_plot
    ...
    if strcmp(cfg.processing_framework, 'joint_2d3d')
        try
            ...
            % Plotting is a presentation step. A renderer/window failure
            % must not discard completed filtering and evaluation results.
            warning('联合可视化失败，继续保存数值结果: %s', ME.message);
        end
    else
    ...
    if ~isempty(cfg.plot_save_dir)
        save_all_open_figures(cfg.plot_save_dir, [], cfg);
    end
    end
    fprintf('可视化与图片保存耗时: %.2f s\n', toc(plot_tic));
end
...
if isfield(est, 'N_total')
    fprintf('║  正式输出点数:    %6d (跨所有事件)            ║\n', sum(est.N_total));
    fprintf('║  二维/三维输出点: %6d / %-6d                  ║\n', sum(est.N2), sum(est.N));
else
...
if exist('metrics_info', 'var') && ~isempty(metrics_info) && isstruct(metrics_info)
    ...
    if ~isempty(truth_count) && isfield(truth_count, 'raw_id_count') && ...
        ...
        passive_split = get_struct_field(truth_count, ...
            'n_passive_time_split_raw_ids', 0);
        passive_instances = get_struct_field(truth_count, ...
            'n_passive_time_split_instances', passive_targets);
        fprintf(['  [真值] 主动目标=%d, 主动拆分实例=%d, ' ...
            '被动目标数据=%d, 纯被动时间实例=%d(拆分编号=%d)\n'], ...
            active_targets, active_instances, passive_targets, ...
            passive_instances, passive_split);
    end
    if isfield(metrics_info, 'two_d') && isfield(metrics_info, 'three_d') && ...
        ...
        if isfield(D2, 'applicable') && ~D2.applicable
            ...
            fprintf(['  [二维] 输出=%d点/%d个二维公开ID, 量测关联利用率=%.2f%%, ' ...
                '量测正确率=%.2f%%, 已关联一致率=%.2f%%, ', ...
                '航迹正确率=%.2f%%\n'], D2.output.n_outputs, D2.output.n_unique_tracks, ...
                100 * D2.association.rate_all_tracks, ...
                100 * D2.measurement_accuracy.rate, ...
                100 * D2.association_consistency.rate, ...
        ...
        fprintf(['  [三维] 输出=%d点/%d个三维公开ID, 量测关联利用率=%.2f%%, ' ...
            '量测正确率=%.2f%%, 已关联一致率=%.2f%%, ', ...
            '航迹正确率=%.2f%%\n'], D3.output.n_outputs, D3.output.n_unique_tracks, ...
            100 * D3.association.rate_all_tracks, ...
            100 * D3.measurement_accuracy.rate, ...
            100 * D3.association_consistency.rate, ...
        ...
        fprintf(['  [总体分域汇总] 输出=%d点/%d个分维ID(二维%d+三维%d；非物理目标数), ' ...
            '关联利用率(全部/曾确认)=%.2f%%/%.2f%%, ', ...
            '量测正确率=%.2f%%, 已关联一致率=%.2f%%, 航迹正确率=%.2f%%\n'], ...
            OA.output.n_outputs, OA.output.n_unique_tracks, ...
            D2.output.n_unique_tracks, D3.output.n_unique_tracks, ...
            100 * OA.association.rate_all_tracks, ...
            100 * OA.association.rate_confirmed_tracks, ...
            100 * OA.measurement_accuracy.rate, ...
            100 * OA.association_consistency.rate, ...
...
if isfield(cfg, 'result_save_file') && ~isempty(cfg.result_save_file)
    ...
    save_mode = get_struct_field(cfg, 'result_save_mode', 'review');
    result_manifest = build_result_manifest(cfg, save_mode);
    if isstruct(metrics_info) && ~isempty(metrics_info)
        metrics_info.result_fingerprint = result_manifest.fingerprint;
    end
    result_save_tic = tic;
    if strcmp(save_mode, 'full')
        save(cfg.result_save_file, 'cfg', 'frames', 'fused_xyz', 'fused_R', 'fused_ids', ...
            ...
            'coherence_info', 'result_manifest', '-v7.3');
    elseif strcmp(save_mode, 'review')
        fprintf('[结果保存] 正在把正式输出整理为review列式历史...\n');
        review_pack_tic = tic;
        review_est = compact_estimate_review(est);
        fprintf('[结果保存] 列式历史完成: %.2f s，正在写入MAT文件...\n', ...
            toc(review_pack_tic));
        payload = struct('cfg', cfg, 'fusion_info', fusion_info, ...
            'metrics_info', metrics_info, 'coherence_info', coherence_info, ...
            'result_manifest', result_manifest, 'frame_times', frame_times, ...
            'est', review_est);
        if strcmp(cfg.processing_framework, 'joint_2d3d')
            payload.joint_events = compact_review_events(joint_events);
        else
            payload.fused_xyz = fused_xyz;
            payload.fused_ids = fused_ids;
        end
        review_mat_version = get_struct_field(cfg, ...
            'result_review_mat_version', 'v7');
        if strcmpi(review_mat_version, 'v7.3')
            save(cfg.result_save_file, '-struct', 'payload', '-v7.3');
        else
            save(cfg.result_save_file, '-struct', 'payload', '-v7');
        end
    else
        payload = struct('cfg', cfg, 'fusion_info', fusion_info, ...
            'metrics_info', metrics_info, 'coherence_info', coherence_info, ...
    ...
    result_file_info = dir(cfg.result_save_file);
    if isempty(result_file_info), result_size_mb = NaN;
    else, result_size_mb = result_file_info(1).bytes / 1024^2;
    end
    fprintf('结果已保存[%s]: %s（%.1f MB，耗时%.2f s）\n', ...
        save_mode, cfg.result_save_file, result_size_mb, toc(result_save_tic));
end
...
fields = {'framework', 'backbone', 'online_causal', 'processing_passes', ...
    ...
    'internal_transition_log', 'output_contract', 'output_id_map', ...
    'output_identity_audit', 'input_accounting', ...
    'measurement_assignment_export_info', 'dimension_transition_log', ...
    'dimension_transition_export_info'};
...
function review = compact_estimate_review(est)
review = compact_estimate_summary(est);
review.saved_history_level = 'review';
review.history_level = 'review';
review.omitted_full_history = true;
review.output_storage = 'columnar_v1';
fields = {'assoc', 'measurement_disposition'};
for k = 1:numel(fields)
    name = fields{k};
    if isfield(est, name), review.(name) = est.(name); end
end
if isfield(est, 'output')
    pack_tic = tic;
    review.output_history = joint_review_output('pack', est.output);
    review.timing.review_output_pack_s = toc(pack_tic);
end
end

function review_events = compact_review_events(events)
review_events = events;
for k = 1:numel(review_events)
    if isfield(review_events(k), 'active')
        remove = intersect({'R_xyz', 'R_ae', 'src'}, ...
            fieldnames(review_events(k).active), 'stable');
        if ~isempty(remove)
            review_events(k).active = rmfield(review_events(k).active, remove);
        end
    end
    if isfield(review_events(k), 'passive')
        remove = intersect({'R_ae', 'src'}, ...
            fieldnames(review_events(k).passive), 'stable');
        if ~isempty(remove)
            review_events(k).passive = rmfield(review_events(k).passive, remove);
        end
    end
end
end

function manifest = build_result_manifest(cfg, save_mode)
manifest = struct('format_version', 2, ...
    'saved_at', datestr(now, 'yyyy-mm-dd HH:MM:SS'), ...
    'save_mode', save_mode, 'processing_framework', ...
    get_struct_field(cfg, 'processing_framework', ''), ...
    'config_fingerprint', lightweight_fingerprint(cfg), ...
    'input_files', collect_input_file_signatures(cfg), ...
    'fingerprint', '');
basis = struct('format_version', manifest.format_version, ...
    'save_mode', manifest.save_mode, ...
    'processing_framework', manifest.processing_framework, ...
    'config_fingerprint', manifest.config_fingerprint, ...
    'input_files', manifest.input_files);
manifest.fingerprint = lightweight_fingerprint(basis);
end

function records = collect_input_file_signatures(cfg)
records = repmat(struct('role', '', 'path', '', 'exists', false, ...
    'bytes', NaN, 'modified_datenum', NaN), 0, 1);
groups = {'active', get_struct_field(cfg, 'active_files', {}); ...
    'passive', get_struct_field(cfg, 'passive_files', {}); ...
    'platform', {get_struct_field(cfg, 'platform_file', '')}};
data_dir = get_struct_field(cfg, 'data_dir', '');
for g = 1:size(groups, 1)
    files = groups{g, 2};
    for q = 1:numel(files)
        path = files{q};
        if isempty(path), continue; end
        path = resolve_data_path(data_dir, path);
        d = dir(path);
        item = struct('role', groups{g, 1}, 'path', path, ...
            'exists', ~isempty(d), 'bytes', NaN, 'modified_datenum', NaN);
        if ~isempty(d)
            item.bytes = d(1).bytes;
            item.modified_datenum = d(1).datenum;
        end
...
function value = lightweight_fingerprint(data)
try
    bytes = uint8(unicode2native(jsonencode(data), 'UTF-8'));
catch
    bytes = uint8(evalc('disp(data)'));
end
a = 1; b = 0; modulus = 65521;
for k = 1:numel(bytes)
    a = mod(a + double(bytes(k)), modulus);
    b = mod(b + a, modulus);
end
value = lower(sprintf('%08x', b * 65536 + a));
end
