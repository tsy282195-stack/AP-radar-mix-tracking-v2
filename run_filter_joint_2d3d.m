...
fprintf(['  三维降维双指标: sigma_r预警/降级=%.0f/%.0fm, ', ...
    '可靠主动距离龄期预警/降级=%.1f/%.1fs, 连续申请=%d\n'], ...
...
for k = 1:K
    ...
    for i = 1:numel(tracks)
        trend_before = tracks(i).los_trend_count;
        tracks(i) = predict_track(tracks(i), t, p);
        stats.los_trend_corrections = stats.los_trend_corrections + ...
            tracks(i).los_trend_count - trend_before;
        if confirm_cycle
            tracks(i).hit_history = shift_window(tracks(i).hit_history, 0);
            tracks(i).evidence_history = shift_window( ...
                tracks(i).evidence_history, NaN);
    ...
    active_suppression_reason = repmat( ...
        {'suppressed_unassociated'}, 1, e.active.n_meas);
    passive_suppression_reason = repmat( ...
        {'suppressed_unassociated'}, 1, e.passive.n_meas);
    ...
    stats.los_trend_corrections = stats.los_trend_corrections + ...
        rebind_info.n_los_trend_corrections;
    ...
    stats.assoc_quality_rejected_edges = stats.assoc_quality_rejected_edges + ...
        active_accept_info.n_quality_low;
    ...
    for q = 1:size(pairs_a, 1)
        ...
        [tracks(ti), range_updated, robust_inflation] = update_track_active( ...
            tracks(ti), e.active, mi, t, sensor, p);
        quality_score = active_accept_info.quality_score(ti, mi);
        quality_margin = active_accept_info.quality_margin(ti, mi);
        tracks(ti).hit_history(end) = 1;
        tracks(ti).evidence_history(end) = quality_score;
        tracks(ti).passive_confirm_streak = 0;
        tracks(ti).passive_quality_sum = 0;
        tracks(ti).passive_quality_count = 0;
        if range_updated
            tracks(ti).active_history(end) = 1;
            tracks(ti).last_active_t = t;
            tracks(ti).last_valid_range_t = max_finite( ...
                tracks(ti).last_valid_range_t, t);
        end
        ...
        assoc.quality_score(end) = quality_score;
        assoc.quality_margin(end) = quality_margin;
        assoc.robust_inflation(end) = robust_inflation;
        stats.active_assigned = stats.active_assigned + 1;
        stats = accumulate_quality_stats(stats, quality_score, robust_inflation);
    end

    active_birth_eligible = false(1, e.active.n_meas);
    for mi = find(~used_a)
        if explained_measurement(cost_a, mi, p.birth_explain_nis)
            stats.active_birth_suppressed = stats.active_birth_suppressed + 1;
            active_suppression_reason{mi} = 'existing_track_explains';
        else
            active_birth_eligible(mi) = true;
        end
    end
    for mi = find(active_birth_eligible)
    ...
    tracks = update_ambiguity_hold_counters(tracks, ...
        accept_info.held_tracks, pairs_p);
    assert_passive_measurement_partition( ...
        pairs_p, detached_used_p, e.passive.n_meas, event_index);
    ...
    stats.passive_stable_locked = stats.passive_stable_locked + ...
        accept_info.n_stable_locked;
    stats.passive_ambiguity_held_tracks = ...
        stats.passive_ambiguity_held_tracks + accept_info.n_ambiguity_held_tracks;
    stats.passive_ambiguity_held_measurements = ...
        stats.passive_ambiguity_held_measurements + ...
        accept_info.n_ambiguity_held_measurements;
    stats.assoc_quality_rejected_edges = stats.assoc_quality_rejected_edges + ...
        accept_info.n_quality_low;
    ...
    for q = 1:size(pairs_p, 1)
        ...
        quality_score = accept_info.quality_score(ti, mi);
        quality_margin = accept_info.quality_margin(ti, mi);
        [tracks(ti), robust_inflation] = update_track_passive( ...
            tracks(ti), e.passive, mi, t, sensor, p);
        [tracks(ti), equivalent_hit, equivalent_quality] = ...
            register_passive_confirm_hit(tracks(ti), t, p, kind, quality_score);
        if equivalent_hit
            tracks(ti).hit_history(end) = 1;
            tracks(ti).evidence_history(end) = equivalent_quality;
        end
        tracks(ti).last_update_t = t;
    ...
    passive_birth_eligible = false(1, e.passive.n_meas);
    % Preserve the raw-NIS birth explanation rule. Measurements held because
    % of an unresolved confirmed-track competition must not create duplicates.
    birth_explain_cost = cost_p;
    for mi = find(~used_p)
        if accept_info.held_measurements(mi)
            stats.passive_birth_suppressed = stats.passive_birth_suppressed + 1;
            passive_suppression_reason{mi} = 'ambiguous_hold';
        elseif ~accept_info.released_measurements(mi) && ...
                explained_measurement(birth_explain_cost, mi, ...
                p.birth_explain_nis)
            stats.passive_birth_suppressed = stats.passive_birth_suppressed + 1;
            passive_suppression_reason{mi} = 'existing_track_explains';
        else
            passive_birth_eligible(mi) = true;
        end
    end
    for mi = find(passive_birth_eligible)
        tr = new_track(next_id, event_index, t, e.passive.ang(:, mi), ...
            e.passive.R_ae(:, :, mi), [], [], false, p);
        tr = update_passive_attributes(tr, e.passive, mi, t, p);
        tr.last_passive_t = t;
    ...
    for i = 1:numel(tracks)
        if ~ismember(tracks(i).id, event_hit_ids)
            ...
            if confirm_cycle && p.passive_confirm_group > 1 && ...
                ...
                tracks(i).passive_quality_sum = 0;
                tracks(i).passive_quality_count = 0;
            end
        end
    end
    ...
    for i = 1:numel(tracks)
        ...
        [count_ready, evidence_ready] = confirmation_evidence_ready(tracks(i), p);
        if count_ready
            stats.track_quality_checks = stats.track_quality_checks + 1;
            if ~evidence_ready
                stats.track_quality_not_ready = stats.track_quality_not_ready + 1;
            end
        end
    ...
    for i = 1:numel(tracks)
        ...
        if ~tracks(i).confirmed
            ...
            quality_expired = mode_is_on(p.track_quality_mode) && ...
                isfinite(tracks(i).quality_wait_since) && ...
                t - tracks(i).quality_wait_since >= p.track_quality_max_wait_s;
            if exhausted || silence > p.tentative_timeout_s || quality_expired
                keep(i) = false;
                stats.deleted_tentative = stats.deleted_tentative + 1;
                stats.track_quality_deleted = stats.track_quality_deleted + ...
                    double(quality_expired);
            end
    ...
    est.measurement_disposition{k} = event_measurement_disposition( ...
        assoc, e, event_index, active_suppression_reason, ...
        passive_suppression_reason);
...
state.stats = struct('active_assigned', 0, 'passive_assigned', 0, ...
    ...
    'passive_stable_locked', 0, 'passive_ambiguity_held_tracks', 0, ...
    'passive_ambiguity_held_measurements', 0, 'los_trend_corrections', 0, ...
    ...
    'assoc_quality_observed', 0, 'assoc_quality_low', 0, ...
    'assoc_quality_sum', 0, 'assoc_quality_min', inf, ...
    'assoc_quality_max', -inf, 'assoc_quality_rejected_edges', 0, ...
    'track_quality_checks', 0, 'track_quality_not_ready', 0, ...
    'track_quality_deleted', 0, 'bearing_robust_downweighted', 0, ...
    'bearing_robust_max_inflation', 1);
state.last_t = NaN;
...
tracks_had_range_time = isempty(state.tracks) || ...
    isfield(state.tracks, 'last_valid_range_t');
companions_had_range_time = isempty(state.companions) || ...
    isfield(state.companions, 'last_valid_range_t');
...
% Schema v8 and earlier used last_active_t as the closest available proxy
% for the last reliable active range update. Preserve continuity when an
% old streaming state is resumed; subsequent accepted range updates refine it.
if ~tracks_had_range_time
    for i = 1:numel(state.tracks)
        state.tracks(i).last_valid_range_t = state.tracks(i).last_active_t;
    end
end
if ~companions_had_range_time
    for i = 1:numel(state.companions)
        state.companions(i).last_valid_range_t = ...
            state.companions(i).last_active_t;
    end
end
...
p.los_trend_enabled = logical(get_cfg(cfg, 'joint_los_trend_enabled', true));
p.los_trend_window = max(2, round(get_cfg(cfg, 'joint_los_trend_window', 50)));
p.los_trend_span_s = get_cfg(cfg, 'joint_los_trend_span_s', 1.0);
p.los_trend_min_n = max(2, round(get_cfg(cfg, 'joint_los_trend_min_n', 10)));
p.los_trend_blend = min(max(get_cfg(cfg, 'joint_los_trend_blend', 0.55), 0), 1);
p.los_trend_min_rate_dps = get_cfg(cfg, 'joint_los_trend_min_rate_dps', 0);
p.los_trend_conf_only = logical(get_cfg(cfg, 'joint_los_trend_conf_only', true));
p.assoc_az_weight = get_cfg(cfg, 'joint_2d_assoc_az_weight', 4.0);
p.assoc_el_weight = get_cfg(cfg, 'joint_2d_assoc_el_weight', 1.0);
p.rate_cost_enabled = logical(get_cfg(cfg, 'joint_2d_rate_cost_enabled', true));
p.rate_cost_weight = get_cfg(cfg, 'joint_2d_rate_cost_weight', 0.50);
p.rate_sigma_dps = get_cfg(cfg, 'joint_2d_rate_sigma_dps', 0.50);
p.rate_cost_cap = get_cfg(cfg, 'joint_2d_rate_cost_cap', 9.0);
p.rate_min_dt_s = get_cfg(cfg, 'joint_2d_rate_min_dt_s', 0.02);
p.rate_min_history = max(2, round(get_cfg(cfg, 'joint_2d_rate_min_history', 3)));
p.stable_lock_enabled = logical(get_cfg(cfg, 'joint_2d_stable_lock_enabled', true));
p.lock_margin = get_cfg(cfg, 'joint_2d_lock_margin', 1.0);
p.lock_max_az_residual_deg = get_cfg(cfg, ...
    'joint_2d_lock_max_az_residual_deg', 0.50);
p.lock_max_rate_cost = get_cfg(cfg, 'joint_2d_lock_max_rate_cost', 4.0);
p.ambiguity_hold_enabled = logical(get_cfg(cfg, ...
    'joint_2d_ambiguity_hold_enabled', true));
p.ambiguity_margin = get_cfg(cfg, 'joint_2d_ambiguity_margin', 0.35);
p.ambiguity_max_events = max(0, round(get_cfg(cfg, ...
    'joint_2d_ambiguity_max_events', 2)));
...
p.assoc_quality_mode = get_cfg_text(cfg, 'assoc_quality_mode', 'off');
p.assoc_quality_margin_nis_active = get_cfg(cfg, ...
    'assoc_quality_margin_nis_active', 2.0);
p.assoc_quality_margin_nis_angle = get_cfg(cfg, ...
    'assoc_quality_margin_nis_angle', 1.0);
p.assoc_quality_min_tentative_active = get_cfg(cfg, ...
    'assoc_quality_min_tentative_active', 0.20);
p.assoc_quality_min_tentative_angle = get_cfg(cfg, ...
    'assoc_quality_min_tentative_angle', 0.20);
p.assoc_quality_min_confirmed = get_cfg(cfg, ...
    'assoc_quality_min_confirmed', 0.05);
p.track_quality_mode = get_cfg_text(cfg, 'track_quality_mode', 'off');
p.track_quality_min_mean = get_cfg(cfg, 'track_quality_min_mean', 0.35);
p.track_quality_min_followup_hits = round(get_cfg(cfg, ...
    'track_quality_min_followup_hits', 2));
p.track_quality_upgrade_min_mean = get_cfg(cfg, ...
    'track_quality_upgrade_min_mean', 0.30);
p.track_quality_upgrade_min_hits = round(get_cfg(cfg, ...
    'track_quality_upgrade_min_hits', 1));
p.track_quality_max_wait_s = get_cfg(cfg, 'track_quality_max_wait_s', 1.0);
p.bearing_robust_mode = get_cfg_text(cfg, 'bearing_robust_mode', 'off');
p.bearing_robust_k = get_cfg(cfg, 'bearing_robust_k', 2.5);
p.bearing_robust_R_max_infl = get_cfg(cfg, 'bearing_robust_R_max_infl', 4.0);
end
...
function [tr, equivalent_hit, equivalent_quality] = register_passive_confirm_hit( ...
        tr, t, p, kind, quality_score)
if nargin < 5 || ~isfinite(quality_score)
    quality_score = 0;
end
if kind == 2 || p.passive_confirm_group <= 1
    equivalent_hit = true;
    equivalent_quality = quality_score;
    tr.passive_confirm_streak = 0;
    tr.passive_quality_sum = 0;
    tr.passive_quality_count = 0;
    return;
end
...
tr.passive_quality_sum = tr.passive_quality_sum + quality_score;
tr.passive_quality_count = tr.passive_quality_count + 1;
equivalent_hit = tr.passive_confirm_streak >= p.passive_confirm_group;
equivalent_quality = NaN;
if equivalent_hit
    equivalent_quality = tr.passive_quality_sum / max(tr.passive_quality_count, 1);
    tr.passive_confirm_streak = 0;
    tr.passive_quality_sum = 0;
    tr.passive_quality_count = 0;
end
...
function tracks = update_ambiguity_hold_counters(tracks, held, pairs)
if isempty(tracks), return; end
held = logical(sized_row(held, numel(tracks), false));
for i = 1:numel(tracks)
    if held(i)
        tracks(i).ambiguity_hold_count = tracks(i).ambiguity_hold_count + 1;
    else
        tracks(i).ambiguity_hold_count = 0;
    end
end
if ~isempty(pairs)
    assigned = unique(pairs(:, 1));
    for i = reshape(assigned, 1, [])
        tracks(i).ambiguity_hold_count = 0;
    end
end
end
...
tr = struct('id', 0, 'birth_event', 0, 'birth_t', NaN, 'confirmed', false, ...
    ...
    'passive_quality_sum', 0, 'passive_quality_count', 0, ...
    'cross_candidate_id', NaN, 'cross_candidate_count', 0, ...
    'hit_history', zeros(1, p.confirm_N), ...
    'evidence_history', nan(1, p.confirm_N), ...
    'quality_wait_since', NaN, ...
    'active_history', zeros(1, max(p.birth3_N, p.upgrade3_N)), ...
    'ambiguity_hold_count', 0, ...
    'los_history', zeros(3, 0), 'los_trend_count', 0, ...
...
if remaining > 0 && tr.angle.valid
    [tr.angle, applied] = apply_los_rate_trend( ...
        tr.angle, tr.los_history, tr.confirmed, p);
    tr.los_trend_count = tr.los_trend_count + double(applied);
end
while remaining > 0
...
[accept_mask, accept_info] = active_2d_acceptance_mask( ...
    C, tracks, meas, t, p);
quality_source = C;
quality_source(~accept_mask) = inf;
[quality_score, quality_margin] = association_quality_matrix( ...
    quality_source, max(p.gate_2d, p.gate_3d), ...
    p.assoc_quality_margin_nis_active);
quality_accept = quality_acceptance_mask(quality_score, tracks, ...
    p.assoc_quality_min_tentative_active, p.assoc_quality_min_confirmed);
accept_info.n_quality_low = nnz(accept_mask & ~quality_accept);
if mode_is_on(p.assoc_quality_mode)
    accept_mask = accept_mask & quality_accept;
    accept_info.released_measurements = any(isfinite(C), 1) & ...
        ~any(accept_mask, 1);
end
accept_info.quality_score = quality_score;
accept_info.quality_margin = quality_margin;
C(~accept_mask) = inf;
...
angles = meas.rae(2:3, :);
for i = 1:numel(tracks)
    requires_space = uses_space_association(tracks(i));
...
row = inf(1, m.n_meas);
requires_space = uses_space_association(tr);
if requires_space
    c3 = active_space_cost_row(tr, m, cache, rae_lambda, xyz_lambda, ...
        1:m.n_meas, p);
    good = isfinite(c3) & c3 <= p.gate_3d;
    row(good) = max(0, c3(good) - p.mode_3d_prior_cost);
    return;
end
if tr.angle.valid
    nu = joint_bearing_residual(m.rae(2:3, :), tr.angle.x([1, 4]));
...
if ~isempty(bearing_local) && cache.valid
    ...
    nu = joint_bearing_residual(m.rae(2:3, jj), cache.zp);
...
requires_space = uses_space_association(tr);
if requires_space
    if isfinite(c3) && c3 <= p.gate_3d
        c = max(0, c3 - p.mode_3d_prior_cost);
    else
        c = inf;
    end
elseif isfinite(c2)
...
if nargin < 8 || isempty(blocked_measurements)
...
[assignment_cost, ranking_info] = passive_assignment_cost( ...
    C, tracks, meas, t, p);
...
quality_source = assignment_cost;
[quality_gate, ~] = finite_gate_for_quality(p);
[quality_score, quality_margin] = association_quality_matrix( ...
    quality_source, quality_gate, ...
    p.assoc_quality_margin_nis_angle);
quality_accept = quality_acceptance_mask(quality_score, tracks, ...
    p.assoc_quality_min_tentative_angle, p.assoc_quality_min_confirmed);
accept_info.n_quality_low = nnz(accept_mask & ~quality_accept);
if mode_is_on(p.assoc_quality_mode)
    accept_mask = accept_mask & quality_accept;
    accept_info.released_measurements = any(isfinite(C), 1) & ...
...
accept_info.quality_margin = quality_margin;
assignment_cost(~accept_mask) = inf;
[pairs, decision] = protected_passive_assignment( ...
    assignment_cost, tracks, ranking_info, p);
accept_info.n_stable_locked = decision.n_stable_locked;
accept_info.n_ambiguity_held_tracks = decision.n_ambiguity_held_tracks;
accept_info.n_ambiguity_held_measurements = ...
    decision.n_ambiguity_held_measurements;
accept_info.held_tracks = decision.held_tracks;
accept_info.held_measurements = decision.held_measurements;
end

function [gate, finite_gate] = finite_gate_for_quality(p)
gate = p.gate_2d;
if isfinite(p.accept_nis_2d), gate = max(gate, p.accept_nis_2d); end
finite_gate = isfinite(gate) && gate > 0;
if ~finite_gate, gate = p.unmatched_cost; end
end

function [pairs, info] = protected_passive_assignment(A, tracks, ranking, p)
n_track = size(A, 1); n_meas = size(A, 2);
info = struct('n_stable_locked', 0, 'n_ambiguity_held_tracks', 0, ...
    'n_ambiguity_held_measurements', 0, ...
    'held_tracks', false(1, n_track), ...
    'held_measurements', false(1, n_meas));
pairs = zeros(0, 2);
if isempty(A) || ~any(isfinite(A(:))), return; end

[margin, ~] = competition_margin_matrix(A);
work = A;
locked = zeros(0, 2);
if p.stable_lock_enabled
    [row_best, row_index] = min(work, [], 2);
    [col_best, col_index] = min(work, [], 1); %#ok<ASGLU>
    for i = 1:n_track
        if ~tracks(i).confirmed || uses_space_association(tracks(i)), continue; end
        j = row_index(i);
        if j < 1 || j > n_meas || ~isfinite(row_best(i)) || ...
                col_index(j) ~= i
            continue;
        end
        if margin(i, j) < p.lock_margin || ...
                ranking.az_residual_deg(i, j) > p.lock_max_az_residual_deg || ...
                ranking.rate_cost(i, j) > p.lock_max_rate_cost
            continue;
        end
        locked(end + 1, :) = [i, j]; %#ok<AGROW>
    end
end
if ~isempty(locked)
    work(locked(:, 1), :) = inf;
    work(:, locked(:, 2)) = inf;
    pairs = locked;
    info.n_stable_locked = size(locked, 1);
end

if p.ambiguity_hold_enabled && p.ambiguity_max_events > 0
    % Recompute competition after removing stable pairs. A pair already
    % locked by continuity must not make the residual problem look ambiguous.
    [margin, competing] = competition_margin_matrix(work);
    for i = 1:n_track
        if ~tracks(i).confirmed || uses_space_association(tracks(i)) || ...
                tracks(i).ambiguity_hold_count >= p.ambiguity_max_events || ...
                any(locked(:, 1) == i)
            continue;
        end
        [best, j] = min(work(i, :));
        if ~isfinite(best) || ~competing(i, j) || ...
                margin(i, j) >= p.ambiguity_margin
            continue;
        end
        candidates = isfinite(work(i, :));
        info.held_tracks(i) = true;
        info.held_measurements = info.held_measurements | candidates;
        % A track-only hold is insufficient: Hungarian could still give one
        % of its competing measurements to another track and create exactly
        % the cross-association this guard is intended to avoid. Stable
        % pairs have already been removed, so withholding these columns for
        % this event cannot disturb a locked pair.
        work(i, :) = inf;
        work(:, candidates) = inf;
    end
end
info.n_ambiguity_held_tracks = nnz(info.held_tracks);
info.n_ambiguity_held_measurements = nnz(info.held_measurements);

remaining = cascade_assignment(work, tracks, p.unmatched_cost);
if ~isempty(remaining), pairs = [pairs; remaining]; end %#ok<AGROW>
end

function [margin, competing] = competition_margin_matrix(C)
margin = inf(size(C));
competing = false(size(C));
for i = 1:size(C, 1)
    for j = find(isfinite(C(i, :)))
        row = C(i, :); row(j) = inf;
        col = C(:, j); col(i) = inf;
        alternative = min(min(row), min(col));
        if isfinite(alternative)
            competing(i, j) = true;
            margin(i, j) = max(0, alternative - C(i, j));
        end
    end
end
end
...
names = {'n_candidate_edges', 'n_accept_edges', 'n_reject_nis', ...
    ...
    'n_quality_low', 'n_stable_locked', 'n_ambiguity_held_tracks', ...
    'n_ambiguity_held_measurements'};
for i = 1:numel(names)
...
total.held_measurements = total.held_measurements | extra.held_measurements;
end
...
info = struct('n_candidate_edges', 0, 'n_accept_edges', 0, ...
    ...
    'n_reject_el', 0, 'n_reject_los', 0, 'n_quality_low', 0, ...
    'n_stable_locked', 0, 'n_ambiguity_held_tracks', 0, ...
    'n_ambiguity_held_measurements', 0, ...
    'released_measurements', false(1, n_meas), ...
    'held_tracks', false(1, 0), ...
    'held_measurements', false(1, n_meas), ...
    'quality_score', zeros(0, n_meas), ...
    'quality_margin', zeros(0, n_meas));
end

function [quality, margin] = association_quality_matrix(C, accept_gate, margin_scale)
quality = nan(size(C));
margin = nan(size(C));
if isempty(C), return; end
margin_scale = max(margin_scale, eps);
[row_first, row_index] = min(C, [], 2);
C_without_row_first = C;
for i = 1:size(C, 1)
    if isfinite(row_first(i))
        C_without_row_first(i, row_index(i)) = inf;
    end
end
row_second = min(C_without_row_first, [], 2);
clear C_without_row_first;
[col_first, col_index] = min(C, [], 1);
C_without_col_first = C;
for j = 1:size(C, 2)
    if isfinite(col_first(j))
        C_without_col_first(col_index(j), j) = inf;
    end
end
col_second = min(C_without_col_first, [], 1);
clear C_without_col_first;
[row, col] = find(isfinite(C));
if isempty(row), return; end
% Keep every per-edge vector explicitly column-shaped.  MATLAB preserves the
% orientation of a source vector for some indexed-vector cases (notably a
% 1-by-N association matrix); mixing that row with find()'s column output
% would otherwise trigger implicit N-by-N expansion in the comparisons.
row = row(:);
col = col(:);
linear = sub2ind(size(C), row, col);
row_alt = reshape(row_first(row), [], 1);
row_index_edge = reshape(row_index(row), [], 1);
row_second_edge = reshape(row_second(row), [], 1);
use_second = row_index_edge == col;
row_alt(use_second) = row_second_edge(use_second);
col_alt = reshape(col_first(col), [], 1);
col_index_edge = reshape(col_index(col), [], 1);
col_second_edge = reshape(col_second(col), [], 1);
use_second = col_index_edge == row;
col_alt(use_second) = col_second_edge(use_second);
alternative = min(row_alt, col_alt);
cost_edge = reshape(C(linear), [], 1);
separation = max(0, alternative - cost_edge);
separation(~isfinite(alternative)) = margin_scale;
margin(linear) = separation;
ambiguity = min(max(separation / margin_scale, 0), 1);
fit = min(max(1 - cost_edge ./ max(accept_gate, eps), 0), 1);
quality(linear) = fit .* (0.5 + 0.5 * ambiguity);
end

function accept = quality_acceptance_mask(quality, tracks, tentative_min, confirmed_min)
accept = true(size(quality));
for i = 1:numel(tracks)
    if tracks(i).confirmed
        threshold = confirmed_min;
    else
        threshold = tentative_min;
    end
    finite = isfinite(quality(i, :));
    accept(i, finite) = quality(i, finite) >= threshold;
end
end

function stats = accumulate_quality_stats(stats, score, inflation)
if isfinite(score)
    stats.assoc_quality_observed = stats.assoc_quality_observed + 1;
    stats.assoc_quality_sum = stats.assoc_quality_sum + score;
    stats.assoc_quality_min = min(stats.assoc_quality_min, score);
    stats.assoc_quality_max = max(stats.assoc_quality_max, score);
    stats.assoc_quality_low = stats.assoc_quality_low + double(score < 0.20);
end
if isfinite(inflation) && inflation > 1 + 1e-12
    stats.bearing_robust_downweighted = stats.bearing_robust_downweighted + 1;
    stats.bearing_robust_max_inflation = max( ...
        stats.bearing_robust_max_inflation, inflation);
end
end
...
for i = 1:numel(tracks)
    ...
    for j = find(candidate(i, :))
        if requires_space
            ...
            joint_residual = joint_bearing_residual(meas.ang(:, j), predicted);
            residual_az = abs(joint_residual(1));
            residual_el = abs(joint_residual(2));
...
joint_residual = joint_bearing_residual(angle, predicted);
residual_az = abs(joint_residual(1));
residual_el = abs(joint_residual(2));
...
function [A, info] = passive_assignment_cost(C, tracks, meas, t, p)
% Keep raw joint NIS in C for statistical gating and birth explanation.
% Pure 2-D ranking deliberately uses weighted marginal residuals so a
% nondiscriminating elevation channel cannot dominate target identity.
A = C;
info = struct('az_residual_deg', inf(size(C)), ...
    'az_cost', inf(size(C)), 'el_cost', inf(size(C)), ...
    'rate_cost', zeros(size(C)));
if isempty(C), return; end
ii = [1, 4];
...
weight_sum = max(p.assoc_az_weight + p.assoc_el_weight, eps);
w_az = 2 * p.assoc_az_weight / weight_sum;
w_el = 2 * p.assoc_el_weight / weight_sum;
for i = 1:numel(tracks)
    if uses_space_association(tracks(i)) || ~tracks(i).angle.valid
        continue;
    end
    ...
    for j = find(isfinite(C(i, :)))
        ...
        S = make_spd(Pz + R);
        nu = joint_bearing_residual(meas.ang(:, j), ...
            tracks(i).angle.x(ii));
        az_cost = nu(1) ^ 2 / max(S(1, 1), eps);
        el_cost = nu(2) ^ 2 / max(S(2, 2), eps);
        rate_cost = passive_rate_consistency_cost( ...
            tracks(i), meas.ang(:, j), t, p, w_az, w_el);
        A(i, j) = w_az * az_cost + w_el * el_cost + ...
            p.rate_cost_weight * rate_cost;
        info.az_residual_deg(i, j) = abs(nu(1));
        info.az_cost(i, j) = az_cost;
        info.el_cost(i, j) = el_cost;
        info.rate_cost(i, j) = rate_cost;
        log_ratio = spd_logdet(S) - R_logdet(j);
        if isfinite(log_ratio) && p.angle_assoc_cov_penalty > 0
            A(i, j) = A(i, j) + ...
                p.angle_assoc_cov_penalty * max(log_ratio, 0);
        end
    end
end
end

function cost = passive_rate_consistency_cost(tr, z, t, p, w_az, w_el)
cost = 0;
if ~p.rate_cost_enabled || ~tr.confirmed || ...
        size(tr.los_history, 2) < p.rate_min_history
    return;
end
last_t = tr.los_history(1, end);
dt = t - last_t;
if ~isfinite(dt) || dt < p.rate_min_dt_s, return; end
[trend, valid] = fit_los_rate(tr.los_history, p.rate_min_history, ...
    p.rate_min_dt_s, p.los_trend_window);
if ~valid, return; end
last_az = tr.los_history(2, end);
last_el = tr.los_history(3, end);
implied = [angle_diff(z(1), wrap_az(last_az)); z(2) - last_el] / dt;
delta = (implied - trend) / max(p.rate_sigma_dps, eps);
cost = min(p.rate_cost_cap, w_az * delta(1) ^ 2 + w_el * delta(2) ^ 2);
if ~isfinite(cost), cost = 0; end
end
...
row = inf(1, m.n_meas);
if uses_space_association(tr)
    c3 = passive_space_cost_row(tr, m, cache, rae_lambda, 1:m.n_meas, p);
    good = isfinite(c3);
    row(good) = max(0, c3(good) - p.mode_3d_prior_cost);
    return;
end
if ~tr.angle.valid, return; end
nu = joint_bearing_residual(m.ang, tr.angle.x([1, 4]));
candidates = nis_candidate_mask(nu, p.gate_2d, cache.angle_lambda, rae_lambda);
for j = find(candidates)
...
nu = joint_bearing_residual(m.ang(:, indices), cache.center);
...
nu = joint_bearing_residual(z, zp);
...
if uses_space_association(tr)
    if tr.space.valid
        [~, S, ~, ok] = space_bearing_stats(tr.space.x, tr.space.P, ...
            t, platform, cfg, m.R_ae(:, :, j));
        if ok
            z3 = space_to_ae(tr.space.x([1, 4, 7]), t, platform, cfg);
            nu = joint_bearing_residual(m.ang(:, j), z3);
            c3 = nu' * (S \ nu);
            if c3 > p.gate_2d, c3 = inf; end
        end
    end
    ...
    if isfinite(c2), c = c2; else, c = inf; end
end
end
...
nu = joint_bearing_residual(z, b.x([1, 4]));
S = make_spd(H * b.P * H' + R);
nis = nu' * (S \ nu);
end

function tf = uses_space_association(tr)
% Keep cost construction and final acceptance on the same state branch.
% Mature 2-D modes never fall back to a stale 3-D shadow when their joint
% azimuth/elevation innovation is outside the 2-D gate.
tf = starts_with(tr.mode, '3d') || (~tr.confirmed && tr.space.valid);
end

function assert_passive_measurement_partition(pairs, blocked, n_meas, event_index)
blocked = logical(sized_row(blocked, n_meas, false));
if isempty(pairs), return; end
indices = reshape(pairs(:, 2), 1, []);
invalid = ~isfinite(indices) | indices < 1 | indices > n_meas | ...
    indices ~= round(indices);
if any(invalid)
    error('run_filter_joint_2d3d:InvalidPassiveAssignment', ...
        'Event %d returned invalid passive measurement indices %s.', ...
        event_index, mat2str(indices(invalid)));
end
if numel(unique(indices)) ~= numel(indices)
    error('run_filter_joint_2d3d:DuplicatePassiveAssignment', ...
        'Event %d assigned one passive measurement to multiple main tracks.', ...
        event_index);
end
reused = indices(blocked(indices));
if ~isempty(reused)
    error('run_filter_joint_2d3d:PassiveMeasurementReused', ...
        ['Event %d reused passive measurements %s after they were already ' ...
        'consumed by detached companion tracks.'], ...
        event_index, mat2str(unique(reused)));
end
end
...
elseif tr.angle.valid
    H = zeros(2, 6); H(1, 1) = 1; H(2, 4) = 1;
    nu = joint_bearing_residual(m.rae(2:3, j), tr.angle.x([1, 4]));
...
elseif tr.space.valid
    ...
    if ok
        nu = joint_bearing_residual(m.rae(2:3, j), zp);
        innovation(1:2) = nu; nis = nu' * (S \ nu); kind = 1;
    end
end
end
...
elseif tr.angle.valid
    H = zeros(2, 6); H(1, 1) = 1; H(2, 4) = 1;
    nu = joint_bearing_residual(z, tr.angle.x([1, 4]));
...
candidates = update_ambiguity_hold_counters( ...
    candidates, accept_info.held_tracks, pairs);
for local_i = 1:numel(eligible)
    companions(eligible(local_i)).ambiguity_hold_count = ...
        candidates(local_i).ambiguity_hold_count;
end
for q = 1:size(pairs, 1)
    ...
    companions(i).angle = update_angle_branch(companions(i).angle, ...
        event.passive.ang(:, mi), event.passive.R_ae(:, :, mi), p);
    companions(i) = update_passive_attributes( ...
        companions(i), event.passive, mi, t, p);
...
function [tr, range_updated, robust_inflation] = update_track_active(tr, m, j, t, sensor, p)
z2 = m.rae(2:3, j);
range_updated = false;
robust_inflation = 1;
if ~tr.angle.valid
    ...
    [tr.angle, robust_inflation] = update_angle_branch( ...
        tr.angle, z2, m.R_ae(:, :, j), p);
end
if m.has_range(j)
    if ~tr.space.valid
...
elseif tr.space.valid
    ...
    if isfinite(nis) && nis <= p.gate_2d
        [tr.space, space_inflation] = update_space_bearing_sensor( ...
            tr.space, z2, m.R_ae(:, :, j), sensor, p);
        robust_inflation = max(robust_inflation, space_inflation);
    end
end
if ~m.has_range(j)
    tr.last_angle_observation_t = t;
...
function [tr, robust_inflation] = update_track_passive(tr, m, j, t, sensor, p)
z = m.ang(:, j); R = m.R_ae(:, :, j);
robust_inflation = 1;
if ~tr.angle.valid
    tr.angle = init_angle_branch(z, R, t, p);
else
    [tr.angle, robust_inflation] = update_angle_branch(tr.angle, z, R, p);
end
if tr.space.valid
    nis = space_bearing_nis_sensor(tr.space, z, R, sensor);
    if isfinite(nis) && nis <= p.gate_2d
        [tr.space, space_inflation] = update_space_bearing_sensor( ...
            tr.space, z, R, sensor, p);
        robust_inflation = max(robust_inflation, space_inflation);
    end
end
tr.last_angle_observation_t = t;
tr = update_passive_attributes(tr, m, j, t, p);
end

function tr = update_passive_attributes(tr, m, j, t, p)
tr = update_passive_los(tr, m, j, t, p);
end

function tr = update_passive_los(tr, m, j, t, p)
tr.los_history = push_los_history(tr.los_history, t, m.ang(:, j), ...
    p.los_trend_window);
end

function hist = push_los_history(hist, t, angle, max_n)
if isempty(angle) || numel(angle) < 2 || any(~isfinite(angle(1:2))) || ~isfinite(t)
    return;
end
if isempty(hist)
    az_unwrapped = angle(1);
else
    az_unwrapped = hist(2, end) + angle_diff(angle(1), wrap_az(hist(2, end)));
end
sample = [t; az_unwrapped; angle(2)];
if ~isempty(hist) && abs(hist(1, end) - t) <= 1e-9
    hist(:, end) = sample;
else
    hist(:, end + 1) = sample;
end
...
function [angle, applied] = apply_los_rate_trend(angle, hist, confirmed, p)
applied = false;
if ~p.los_trend_enabled || ~angle.valid || ...
        (p.los_trend_conf_only && ~confirmed) || size(hist, 2) < p.los_trend_min_n
    return;
end
[trend, valid] = fit_los_rate(hist, p.los_trend_min_n, ...
    p.los_trend_span_s, p.los_trend_window);
if ~valid || norm(trend) < p.los_trend_min_rate_dps, return; end
idx = [2, 5];
corrected = (1 - p.los_trend_blend) * angle.x(idx) + ...
    p.los_trend_blend * trend;
angle.x(idx) = corrected;
angle.xm(idx, :) = repmat(corrected, 1, size(angle.xm, 2));
applied = true;
end

function [trend, valid] = fit_los_rate(hist, min_n, min_span_s, max_n)
trend = nan(2, 1); valid = false;
if size(hist, 2) < min_n, return; end
if size(hist, 2) > max_n
    hist = hist(:, end - max_n + 1:end);
end
t = hist(1, :);
if t(end) - t(1) < min_span_s, return; end
tc = t - mean(t); denom = sum(tc .^ 2);
if denom <= 0, return; end
trend = [sum(tc .* (hist(2, :) - mean(hist(2, :)))); ...
    sum(tc .* (hist(3, :) - mean(hist(3, :))))] / denom;
valid = all(isfinite(trend));
end
...
function [b, robust_inflation] = update_angle_branch(b, z, R, p)
H = zeros(2, 6); H(1, 1) = 1; H(2, 4) = 1;
[R_update, robust_inflation] = bearing_robust_covariance( ...
    [angle_diff(z(1), b.x(1)); z(2) - b.x(4)], ...
    H * b.P * H', R, p);
like = zeros(2, 1); nis_model = nan(2, 1);
for j = 1:2
    x = b.xm(:, j); P = b.Pm(:, :, j);
    nu = joint_bearing_residual(z, x([1, 4]));
    S = make_spd(H * P * H' + R_update); K = (P * H') / S;
    x = x + K * nu; x(1) = wrap_az(x(1));
    I = eye(6); P = (I - K * H) * P * (I - K * H)' + ...
        K * R_update * K';
...
function [b, robust_inflation] = update_space_bearing_sensor(b, z, R, sensor, p)
[zp0, S0, ~, ok0] = space_bearing_moments(b.x, b.P, sensor);
if ok0
    nu0 = [angle_diff(z(1), zp0(1)); z(2) - zp0(2)];
    [R_update, robust_inflation] = bearing_robust_covariance(nu0, S0, R, p);
else
    R_update = R;
    robust_inflation = 1;
end
...
for j = 1:2
    x = b.xm(:, j); P = b.Pm(:, :, j);
    [zp, S, Pxz, ok] = space_bearing_stats_sensor(x, P, sensor, R_update);
...
function [R_update, inflation] = bearing_robust_covariance(nu, S0, R, p)
R = make_spd(R);
S_raw = make_spd(S0 + R);
nis = nu' * (S_raw \ nu);
standardized = sqrt(max(nis, 0) / max(numel(nu), 1));
inflation = 1;
if isfinite(standardized) && standardized > p.bearing_robust_k
    inflation = min((standardized / p.bearing_robust_k)^2, ...
        p.bearing_robust_R_max_infl);
end
if mode_is_on(p.bearing_robust_mode)
    R_update = make_spd(inflation * R);
else
    R_update = R;
end
end
...
if has_external
    ...
    if mode_is_on(p.track_quality_mode)
        external_quality_ready = logical(sized_row(field_or( ...
            event.external_3d, 'quality_ready', false), n_external, false));
        external_ready = external_ready & external_quality_ready;
    end
...
for i = 1:numel(tracks)
    ...
    bearing_residual = joint_bearing_residual( ...
        external_ang, tracks(i).angle.x([1, 4]));
    sep = hypot(bearing_residual(1, :), bearing_residual(2, :));
    ...
    candidates = find(isfinite(sep) & sep <= p.cross_suppress_angle_deg & ...
        rate_ok);
...
for i = 1:numel(companions)
    ...
    trend_before = companions(i).los_trend_count;
    companions(i) = predict_companion(companions(i), t, p, predict_space);
    rebind_info.n_los_trend_corrections = ...
        rebind_info.n_los_trend_corrections + ...
        companions(i).los_trend_count - trend_before;
...
for q = 1:n
    ...
    if ~known(q)
        ...
        % Mature 3-D last_active_t is refreshed only by an accepted active
        % Cartesian/range update, so it is the reliable range timestamp.
        c.last_valid_range_t = last_active(q);
    ...
    companions(i).last_valid_range_t = max_finite( ...
        companions(i).last_valid_range_t, last_active(q));
...
update_kind = sized_row(field_or(ext, 'update_kind', zeros(1, 0)), ...
    numel(update_ids), 0);
update_meas = struct('ang', update_ang);
...
for q = 1:numel(update_ids)
    ...
    if ~companions(i).angle.valid
        ...
        companions(i).angle = update_angle_branch( ...
            companions(i).angle, update_ang(:, q), update_R(:, :, q), p);
    end
    companions(i).last_update_t = t;
    companions(i).last_angle_observation_t = t;
    if update_kind(q) == 2
        ...
        % The mature 3-D signature already contains this accepted passive
        % observation and was mirrored above. Update only the LOS history
        % here so the same accepted angle observation is not counted twice.
        companions(i) = update_passive_los( ...
            companions(i), update_meas, q, t, p);
        companions(i).last_passive_t = t;
    end
end
...
for i = 1:numel(companions)
    ...
    if quality_cycle || needs_confirmation_eval
        companions(i).quality = assess_quality(companions(i), t, sensor, p);
...
ready = logical(sized_row(field_or(ext, 'dimension_ready', ...
    false(1, numel(ids))), numel(ids), false));
if mode_is_on(p.track_quality_mode)
    quality_ready = logical(sized_row(field_or(ext, 'quality_ready', ...
        false(1, numel(ids))), numel(ids), false));
    ready = ready & quality_ready;
end
...
for a = 1:numel(eligible)
    ...
    for b = 1:numel(unknown)
        ...
        bearing_residual = joint_bearing_residual( ...
            angles(:, q), tr.angle.x([1, 4]));
        continuing_candidate = tr.rebind_candidate_id == ids(q) && ...
            any(tr.rebind_history);
        if ~continuing_candidate && hypot(bearing_residual(1), ...
                bearing_residual(2)) > p.cross_suppress_angle_deg
...
nu = joint_bearing_residual(z2, z3);
...
for q = reshape(order, 1, [])
    ...
    companions(i).los_history = records(q).track.los_history;
    companions(i).los_trend_count = companions(i).los_trend_count + ...
        records(q).track.los_trend_count;
...
template = struct('external_3d_id', NaN, 'local_2d_id', NaN, ...
    ...
    'last_active_t', NaN, 'last_valid_range_t', NaN, ...
    'active_evidence_count', 0, 'active_evidence_window', p.upgrade3_N, ...
...
for i = 1:numel(companions)
    ...
    s.last_active_t = tr.last_active_t;
    s.last_valid_range_t = tr.last_valid_range_t;
    active_window = tr.active_history( ...
        max(1, end - p.upgrade3_N + 1):end);
    s.active_evidence_count = sum(active_window);
    s.active_evidence_window = numel(active_window);
    s.up_count = tr.up_count;
    s.down_count = tr.down_count;
...
for i = 1:numel(tracks)
    ...
    for j = i + 1:numel(tracks)
        ...
        bearing_residual = joint_bearing_residual( ...
            tracks(i).angle.x([1, 4]), tracks(j).angle.x([1, 4]));
        da = hypot(bearing_residual(1), bearing_residual(2));
...
for q = 1:numel(keep.evidence_history)
    candidates = [keep.evidence_history(q), drop.evidence_history(q)];
    candidates = candidates(isfinite(candidates));
    if isempty(candidates)
        keep.evidence_history(q) = NaN;
    else
        keep.evidence_history(q) = max(candidates);
    end
end
keep.active_history = max(keep.active_history, drop.active_history);
if drop.passive_confirm_streak > keep.passive_confirm_streak
    keep.passive_quality_sum = drop.passive_quality_sum;
    keep.passive_quality_count = drop.passive_quality_count;
end
keep.passive_confirm_streak = max(keep.passive_confirm_streak, ...
    drop.passive_confirm_streak);
keep.quality_wait_since = min_finite( ...
    keep.quality_wait_since, drop.quality_wait_since);
...
use_drop_los_history = size(drop.los_history, 2) > size(keep.los_history, 2) || ...
    drop.last_angle_observation_t > keep.last_angle_observation_t;
keep.last_angle_observation_t = max_finite( ...
    keep.last_angle_observation_t, drop.last_angle_observation_t);
if use_drop_los_history
    keep.los_history = drop.los_history;
end
keep.los_trend_count = keep.los_trend_count + drop.los_trend_count;
...
[~, evidence_ready] = confirmation_evidence_ready(tr, p);
if ready_to_confirm && ~evidence_ready
    if ~isfinite(tr.quality_wait_since)
        tr.quality_wait_since = t;
    end
elseif evidence_ready
    tr.quality_wait_since = NaN;
end
if quality_cycle || ready_to_confirm
    tr.quality = assess_quality(tr, t, sensor, p);
end
if ready_to_confirm && (~mode_is_on(p.track_quality_mode) || evidence_ready)
    tr.confirmed = true;
    tr.quality_wait_since = NaN;
...
function [count_ready, evidence_ready, mean_quality, n_quality] = ...
        confirmation_evidence_ready(tr, p)
count_ready = ~tr.confirmed && sum(tr.hit_history) >= p.confirm_M;
mask = tr.hit_history > 0 & isfinite(tr.evidence_history);
n_quality = nnz(mask);
if n_quality > 0
    mean_quality = mean(tr.evidence_history(mask));
else
    mean_quality = NaN;
end
evidence_ready = n_quality >= p.track_quality_min_followup_hits && ...
    isfinite(mean_quality) && mean_quality >= p.track_quality_min_mean;
end
...
if tr.space.valid
    ...
    % radial 1-sigma range uncertainty and age of the last reliable active
    % range update. Position95, relative uncertainty and NIS remain purely
    % diagnostic so maneuver/model mismatch cannot directly force a drop.
    ...
    q.down3d = q.degrade_request; % compatibility alias for existing manager
...
q = struct('valid2d', false, 'valid3d', false, 'warn3d', false, ...
    ...
    'switch_consistent', false, 'observed2d', false, ...
...
s.mode = tr.mode; s.last_update_t = tr.last_update_t;
...
a = struct('id', zeros(1, 0), 'type', {cell(1, 0)}, 'meas_index', zeros(1, 0), ...
    ...
    'quality_score', zeros(1, 0), 'quality_margin', zeros(1, 0), ...
    'robust_inflation', zeros(1, 0));
end
...
a.quality_score(end + 1) = NaN;
a.quality_margin(end + 1) = NaN;
a.robust_inflation(end + 1) = 1;
end
...
a.quality_score = [a.quality_score, b.quality_score];
a.quality_margin = [a.quality_margin, b.quality_margin];
a.robust_inflation = [a.robust_inflation, b.robust_inflation];
end
...
function d = event_measurement_disposition( ...
        a, e, event_index, active_reason, passive_reason)
...
for q = 1:numel(a.id)
    ...
    if association_type_matches(a.type{q}, 'active') && ...
            mi >= 1 && mi <= e.active.n_meas
        active_suppressed(mi) = false;
    elseif association_type_matches(a.type{q}, 'passive') && ...
            mi >= 1 && mi <= e.passive.n_meas
...
function tf = mode_is_on(value)
tf = ischar(value) && strcmpi(value, 'on');
end
...
function v = get_cfg_text(cfg, name, fallback)
v = get_cfg(cfg, name, fallback);
if isstring(v) && isscalar(v), v = char(v); end
if ~ischar(v), v = fallback; end
v = lower(strtrim(v));
end
