...
for k = 1:n_frames
    ...
    idx = p + (1:M);
    act_xyz(:, idx) = Z;
    act_meas_t(idx) = normalize_time_vector( ...
        get_frame_field(frames(k), 'active_t', []), M, frames(k).t_sec);
    if strcmp(active_time_mode, 'plot')
        act_t(idx) = act_meas_t(idx);
    else
        act_t(idx) = frames(k).t_sec * ones(1, M);
    end
    frame_rae = get_frame_field(frames(k), 'active_rae', []);
    if size(frame_rae, 1) >= 3 && size(frame_rae, 2) >= M
        act_rae(:, idx) = frame_rae(1:3, 1:M);
    end
...
act_meas_t = act_meas_t(act_good); act_rae = act_rae(:, act_good);
...
if isempty(all_t)
    ...
    event_meta = struct('has_active', {}, 'has_passive', {}, 'miss_cycle', {}, ...
        ...
        't_start', {}, 't_end', {}, 'active_rae', {}, 'active_t', {});
...
event_meta = repmat(struct('has_active', false, 'has_passive', false, ...
    ...
    't_start', NaN, 't_end', NaN, 'active_rae', zeros(3, 0), ...
    'active_t', zeros(1, 0)), K, 1);
...
for k = 1:K
    ...
    event_meta(k).active_rae = act_rae(:, a_local);
    event_meta(k).active_t = act_meas_t(a_local);
end
...
function M = normalize_matrix(M, rows, cols, fill_value)
if isempty(M)
    M = fill_value * ones(rows, cols);
    return;
end
if size(M, 1) < rows
    M(end + 1:rows, :) = fill_value;
end
if size(M, 2) < cols
    M(:, end + 1:cols) = fill_value;
end
M = M(1:rows, 1:cols);
end
