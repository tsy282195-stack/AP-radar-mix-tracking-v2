clear play_track_filter plot_track_filter_smooth plot_pseudo_truth smooth_filter_tracks
rehash

TRACK_IDS    = [1030 1013 1028 1011 1001 1005 1019 1008];          % 3D ID从1递增，2D ID从1001递增；填[]=全部正式输出航迹
...
TRUTH_SELECT_MODE = 'all_touched'; % dominant=主匹配；all_touch/all_touched=画全部触达真值
...
if ~have && ~isempty(RESULT_MAT)
    if exist(RESULT_MAT, 'file')
        ...
        if isfield(Sload, 'metrics_info'), metrics_info = Sload.metrics_info; end
        if isfield(Sload, 'result_manifest'), result_manifest = Sload.result_manifest; end
        have = exist('est', 'var') && exist('frame_times', 'var');
    else
...
if exist('result_manifest', 'var') && isstruct(result_manifest)
    saved_mode = 'unknown';
    if isfield(result_manifest, 'save_mode'), saved_mode = result_manifest.save_mode; end
    fprintf('[结果文件] 格式版本=%g，保存级别=%s，签名=%s\n', ...
        result_manifest.format_version, saved_mode, result_manifest.fingerprint);
    if exist('metrics_info', 'var') && isstruct(metrics_info) && ...
            isfield(metrics_info, 'result_fingerprint') && ...
            ~strcmp(metrics_info.result_fingerprint, result_manifest.fingerprint)
        error('评价结果签名与结果文件不一致；请勿混用不同运行产生的metrics_info。');
    end
end
if isfield(est, 'framework') && strncmp(est.framework, 'joint_2d3d', 10)
    if ~isfield(est, 'output') && ~isfield(est, 'output_history')
        level = 'summary';
        if isfield(est, 'saved_history_level'), level = est.saved_history_level; end
        error(['当前结果为%s级别，不含正式输出历史，无法执行view_track。' ...
            '请把cfg.result_save_mode设为''review''或''full''后重新运行。'], level);
    end
