...
% target_id is never used by filtering here. Active truth may be split by
% spatial/time discontinuities; pure-passive truth uses time gaps only.
...
if ~split_enabled
    ...
    [labels, summary] = apply_passive_only_time_split( ...
        events, labels, summary, all_active_ids, cfg);
...
if get_cfg(cfg, 'passive_truth_id_split_enabled', true) ~= 0
    max_gap = get_cfg(cfg, 'passive_truth_id_split_max_gap_s', 10);
else
    max_gap = inf;
end
raw_ids = unique(raw(isfinite(raw)));
...
unmatched_split_raw = 0;
for rid = reshape(unique(unmatched_raw), 1, [])
    unmatched_split_raw = unmatched_split_raw + (nnz(unmatched_raw == rid) > 1);
end
info = struct('raw_ids', raw_ids, 'n_segments', numel(segments), ...
    ...
    'n_unmatched_split_raw_ids', unmatched_split_raw, ...
    'unmatched_raw_ids', unmatched_raw, ...
    'unmatched_instances', unmatched_instance, ...
    'segment_raw_ids', [segments.raw_id], ...
...
summary.n_split_raw_ids = reference_info.n_split_raw_ids + ...
    info.n_unmatched_split_raw_ids;
...
summary.n_passive_segmented_raw_ids = info.n_unmatched_split_raw_ids;
summary.n_passive_time_split_raw_ids = info.n_unmatched_split_raw_ids;
summary.n_passive_time_split_instances = info.n_unmatched_segments;
...
summary.passive_truth_split_enabled = get_cfg( ...
    cfg, 'passive_truth_id_split_enabled', true) ~= 0;
summary.passive_max_gap_s = get_cfg( ...
    cfg, 'passive_truth_id_split_max_gap_s', 10);
...
function [labels, summary] = apply_passive_only_time_split( ...
        events, labels, summary, active_raw_ids, cfg)
% Split only truth that exists in the physical-passive AE source.  The
% split criterion is the gap between two consecutive time-sorted AE plots;
% angle distance and filter association never enter this evaluation label.
enabled = get_cfg(cfg, 'passive_truth_id_split_enabled', true) ~= 0;
max_gap = get_cfg(cfg, 'passive_truth_id_split_max_gap_s', 10);
summary.passive_truth_split_enabled = enabled;
summary.passive_max_gap_s = max_gap;
summary.n_passive_time_split_raw_ids = 0;
summary.n_passive_time_split_instances = 0;
if ~enabled
    return;
end

n_total = sum(arrayfun(@(e) e.passive.n_meas, events));
rec_event = zeros(1, n_total);
rec_index = zeros(1, n_total);
raw = nan(1, n_total);
times = nan(1, n_total);
p = 0;
for k = 1:numel(events)
    ...
    if n == 0, continue; end
    ids = sized_row(events(k).passive.ids, n, NaN);
    kind = sized_row(field_or(events(k).passive, ...
        'kind', ones(1, n)), n, 1);
    tt = measurement_times(events(k).passive.t_sec, n, events(k).t_sec);
    % "Pure passive" means that this raw truth ID has no active RAE or
    % active-file AE-only observation anywhere in the evaluation interval.
    idx = find(kind == 1 & isfinite(ids) & isfinite(tt) & ...
        ~ismember(ids, active_raw_ids));
...
rec_event = rec_event(1:p);
rec_index = rec_index(1:p);
raw = raw(1:p);
times = times(1:p);
...
existing = collect_finite_truth_labels(labels);
if isempty(existing)
    next_label = 1;
else
    next_label = ceil(max(existing)) + 1;
end
new_labels = zeros(1, 0);
new_raw = zeros(1, 0);
new_instance = zeros(1, 0);
new_text = cell(1, 0);
split_raw_count = 0;
segment_count = 0;

for rid = reshape(unique(raw), 1, [])
    jj = find(raw == rid);
    [~, order] = sort(times(jj));
    jj = jj(order);
    cuts = [1, find(diff(times(jj)) > max_gap) + 1, numel(jj) + 1];
    n_part = numel(cuts) - 1;
    segment_count = segment_count + n_part;
    if n_part <= 1
        continue;
    end
    split_raw_count = split_raw_count + 1;
    for part = 1:n_part
        records = jj(cuts(part):cuts(part + 1) - 1);
        key = next_label;
        next_label = next_label + 1;
        for q = reshape(records, 1, [])
            labels.passive{rec_event(q)}(rec_index(q)) = key;
        end
        new_labels(end + 1) = key; %#ok<AGROW>
        new_raw(end + 1) = rid; %#ok<AGROW>
        new_instance(end + 1) = part; %#ok<AGROW>
        new_text{end + 1} = sprintf( ...
            '%g(passive_raw=%g,part=%d)', key, rid, part); %#ok<AGROW>
    end
end

used_labels = collect_finite_truth_labels(labels);
old_labels = reshape(field_or(summary, 'instance_labels', zeros(1, 0)), 1, []);
old_raw = sized_row(field_or(summary, 'key_raw_id', []), numel(old_labels), NaN);
old_instance = sized_row(field_or(summary, 'key_instance', []), ...
    numel(old_labels), 1);
old_text = field_or(summary, 'instance_label_text', cell(1, 0));
if ~iscell(old_text), old_text = cell(1, 0); end
old_text = reshape(old_text, 1, []);
if numel(old_text) < numel(old_labels)
    old_text(end + 1:numel(old_labels)) = {''};
elseif numel(old_text) > numel(old_labels)
    old_text = old_text(1:numel(old_labels));
end
keep = ismember(old_labels, used_labels);
summary.instance_labels = [old_labels(keep), new_labels];
summary.key_raw_id = [old_raw(keep), new_raw];
summary.key_instance = [old_instance(keep), new_instance];
summary.instance_label_text = [old_text(keep), new_text];
summary.instance_count = numel(summary.instance_labels);
summary.n_unified_truth_instances = summary.instance_count;
summary.n_passive_time_split_raw_ids = split_raw_count;
summary.n_passive_time_split_instances = segment_count;
summary.n_passive_segmented_raw_ids = split_raw_count;
summary.n_passive_mapping_segments = segment_count;
summary.n_split_raw_ids = field_or(summary, ...
    'n_active_split_raw_ids', 0) + split_raw_count;

pure_labels = zeros(1, 0);
for q = 1:p
    key = labels.passive{rec_event(q)}(rec_index(q));
    if isfinite(key), pure_labels(end + 1) = key; end %#ok<AGROW>
end
pure_labels = unique(pure_labels);
summary.passive_only_instance_labels = pure_labels;
active_angle_labels = reshape(field_or(summary, ...
    'active_angle_only_instance_labels', zeros(1, 0)), 1, []);
summary.angle_only_instance_labels = unique([active_angle_labels, pure_labels]);
summary.enabled = true;
summary.label_basis = 'active_split_plus_passive_time_split';
end

function values = collect_finite_truth_labels(labels)
values = zeros(1, 0);
for k = 1:numel(labels.active)
    values = [values, reshape(labels.active{k}, 1, []), ... %#ok<AGROW>
        reshape(labels.passive{k}, 1, [])]; %#ok<AGROW>
end
