...
if K == 0
    ...
    est.track_quality_stats = empty_track_quality_stats();
...
assoc_quality_mode = get_cfg_text_field(cfg, 'assoc_quality_mode', 'off');
assoc_quality_margin_scale = get_cfg_field(cfg, 'assoc_quality_margin_nis_active', 2.0);
assoc_quality_min_tent = get_cfg_field(cfg, ...
    'assoc_quality_min_tentative_active', 0.20);
assoc_quality_min_conf = get_cfg_field(cfg, 'assoc_quality_min_confirmed', 0.05);
track_quality_mode = get_cfg_text_field(cfg, 'track_quality_mode', 'off');
track_quality_min_mean = get_cfg_field(cfg, 'track_quality_min_mean', 0.35);
track_quality_min_hits = round(get_cfg_field(cfg, ...
    'track_quality_min_followup_hits', 2));
track_quality_max_wait_s = get_cfg_field(cfg, 'track_quality_max_wait_s', 1.0);
...
if passive_bearing_enabled
    fprintf('bearing-only更新: gate=%.1f, NIS门=%.1f, 单次命中直接记主动确认=%d\n', ...
        passive_bearing_gate, passive_bearing_nis_gate, passive_bearing_confirm_hit);
    if joint_extension_enabled
        fprintf(['跨维起始辅助: 连续%d次bearing命中折算1次等效确认命中；' ...
            '主动/等效被动命中共同参与二维与三维M/N确认。\n'], ...
            joint_passive_confirm_hits);
    end
...
track_quality_stats = empty_track_quality_stats();
...
for k = 1:K
    ...
    if trk.N > 0
        ...
        if count_confirm_cycle
            ...
            pred.Q   = [trk.Q(2:end, :); nan(1, trk.N)];
        else
            pred.age = trk.age;
    ...
    quality_mat = nan(pred.N, Mg);
    quality_margin_mat = nan(pred.N, Mg);
    if Mg > 0 && pred.N > 0
        track_accept_nis = assoc_accept_nis * ones(1, pred.N);
        ...
        active_assoc_stats.n_edges_over_accept_nis = active_assoc_stats.n_edges_over_accept_nis + ...
            nnz(isfinite(nis_mat) & nis_mat > track_accept_nis(:));
        cost_mat(nis_mat > track_accept_nis(:)) = inf;
        quality_source = nis_mat;
        quality_source(nis_mat > track_accept_nis(:)) = inf;
        [quality_mat, quality_margin_mat] = active_association_quality( ...
            quality_source, track_accept_nis, assoc_quality_margin_scale);
        quality_accept = active_quality_acceptance(quality_mat, pred.conf, ...
            assoc_quality_min_tent, assoc_quality_min_conf);
        active_assoc_stats.n_quality_low_edges = active_assoc_stats.n_quality_low_edges + ...
            nnz(isfinite(cost_mat) & ~quality_accept);
        if mode_is_on(assoc_quality_mode)
            cost_mat(~quality_accept) = inf;
        end
        secondary_nis_mat = nis_mat;
        if mode_is_on(assoc_quality_mode)
            secondary_nis_mat(~quality_accept) = inf;
        end
        ...
        for stage_code = 1:3
            ...
            if meas_fuse_enabled && ~isempty(added_tracks)
                [track_meas, available_meas, failed_mi, failed_parent, pair_nis_used, pair_dist_used] = ...
                    ...
                    secondary_nis_mat, z_gated, gated_idx, R_k_meas, R_default, ...
                    track_accept_nis, meas_fuse_cluster_gamma, meas_fuse_cluster_dist_m);
        ...
        [track_meas, primary_stage, available_meas, banned_edges, rejected_pairs, released_pairs] = ...
            ...
            R_min_diag, meas_fuse_R_scale, meas_fuse_weight, track_accept_nis);
        ...
        for mi = find(available_meas)
            ...
            broad_conflict = isfinite(nis_mat(:, mi).') & ...
                nis_mat(:, mi).' > track_accept_nis & nis_mat(:, mi).' <= cost_unmatched;
    ...
    assoc_quality_score = nan(1, 0);
    assoc_quality_margin = nan(1, 0);
    assoc_robust_inflation = ones(1, 0);
    ...
    for ti = 1:pred.N
        ...
        track_accept_limit = acceptance_gate_at(track_accept_nis, ti);
        if ~g.valid || ~isfinite(g.nis) || g.nis > track_accept_limit
        ...
        z_bar = g.z_bar;
        primary_mi = J(1);
        quality_score = quality_mat(ti, primary_mi);
        quality_margin_value = quality_margin_mat(ti, primary_mi);
        ...
        robust_inflation = max(diag(R_upd) ./ max(diag(R_bar), realmin));
        ...
        buf = pred.innov{ti};
        buf = [buf, nu_dom]; %#ok<AGROW>
        weight_buf = pred.innov_weight{ti};
        sample_weight = association_sample_weight(quality_score, robust_inflation, cfg);
        weight_buf = [weight_buf, sample_weight]; %#ok<AGROW>
        if size(buf, 2) > cfg.adapt_R_window
            buf = buf(:, end - cfg.adapt_R_window + 1 : end);
            weight_buf = weight_buf(end - cfg.adapt_R_window + 1 : end);
        end
        ...
        trk.innov{ti}   = buf;
        trk.innov_weight{ti} = weight_buf;
        trk.R{ti}       = R_use;
        active_assoc_stats = accumulate_active_quality_stats(active_assoc_stats, ...
            quality_score, robust_inflation, sample_weight);
        ...
        for jj = 1:nJ
            ...
            assoc_quality_score(end+1) = quality_score; %#ok<AGROW>
            assoc_quality_margin(end+1) = quality_margin_value; %#ok<AGROW>
            assoc_robust_inflation(end+1) = robust_inflation; %#ok<AGROW>
        end
    end
    ...
    if N_birth > 0
        ...
        b.Q   = nan(cfg.N_confirm, N_birth);    % 出生种子不虚构关联可信度
        b.innov = cell(1, N_birth);
        b.innov_weight = cell(1, N_birth);
        b.R   = cell(1, N_birth);
        ...
        b.quality_wait_since = nan(1, N_birth);
        b.N = N_birth;
        ...
        for bb = 1:N_birth
            ...
            assoc_quality_score(end+1) = NaN;              %#ok<AGROW>
            assoc_quality_margin(end+1) = NaN;             %#ok<AGROW>
            assoc_robust_inflation(end+1) = 1;             %#ok<AGROW>
            next_id = next_id + 1;
            b.innov{bb} = zeros(z_dim, 0);
            b.innov_weight{bb} = zeros(1, 0);
    ...
    if joint_extension_enabled && passive_bearing_enabled && trk.N > 0
        ...
        direct_quality = passive_quality_for_ids(passive_detail_k, direct_ids);
        grouped_quality = passive_quality_for_ids(passive_detail_k, grouped_ids);
        [trk, passive_confirm_state, n_direct] = apply_passive_equivalent_hits( ...
            trk, passive_confirm_state, direct_ids, direct_quality, ...
        ...
        [trk, passive_confirm_state, n_grouped] = apply_passive_equivalent_hits( ...
            trk, passive_confirm_state, grouped_ids, grouped_quality, ...
    ...
    if trk.N > 1
        ...
        trk = dedup_tracks(trk, pos_idx, vel_idx, merge_pos_dist_m, ...
                           dedup_vel_angle, dedup_min_speed, cfg);
        ...
        for tr = 1:trk.N
            if trk.conf(tr) == 1
                ...
                if n_hits >= cfg.M_confirm
                    [quality_ready, quality_mean, quality_hits] = ...
                        track_quality_status(trk.Q(:, tr), trk.S(:, tr), ...
                        track_quality_min_hits, track_quality_min_mean);
                    track_quality_stats.n_checks = track_quality_stats.n_checks + 1;
                    track_quality_stats.quality_sum = track_quality_stats.quality_sum + ...
                        finite_or_zero(quality_mean);
                    track_quality_stats.quality_hit_sum = ...
                        track_quality_stats.quality_hit_sum + quality_hits;
                    if ~quality_ready
                        track_quality_stats.n_not_ready = track_quality_stats.n_not_ready + 1;
                        if ~isfinite(trk.quality_wait_since(tr))
                            trk.quality_wait_since(tr) = t_k;
                        end
                        if mode_is_on(track_quality_mode) && ...
                                t_k - trk.quality_wait_since(tr) >= track_quality_max_wait_s
                            quality_delete(tr) = true;
                            continue;
                        end
                    else
                        trk.quality_wait_since(tr) = NaN;
                    end
                    if mode_is_on(track_quality_mode) && ~quality_ready
                        continue;
                    end
                    trk.conf(tr) = 1;
    ...
    if any(quality_delete)
        trk_before_quality_delete = trk;
        trk = trk_subset(trk, ~quality_delete);
        n_quality_deleted = nnz(quality_delete);
        track_quality_stats.n_deleted = track_quality_stats.n_deleted + n_quality_deleted;
        delete_stats.n_tentative_quality = delete_stats.n_tentative_quality + n_quality_deleted;
        [n_nis_removed, n_secondary_removed] = removed_birth_kind_counts( ...
            trk_before_quality_delete, trk, BIRTH_NIS_CONFLICT, BIRTH_SECONDARY_CONFLICT);
        birth_stats.n_nis_conflict_deleted = ...
            birth_stats.n_nis_conflict_deleted + n_nis_removed;
        birth_stats.n_secondary_conflict_deleted = ...
            birth_stats.n_secondary_conflict_deleted + n_secondary_removed;
        idx_confirmed = find(trk.conf == 1);
    end
    ...
    est.tracks{k} = struct('m', trk.m, 'P', trk.P, 'w', trk.w, ...
                           'L', trk.L, 'S', trk.S, 'Q', trk.Q, 'R', {trk.R}, ...
                           ...
                           'quality_wait_since', trk.quality_wait_since, ...
                           'imm_mu', imm_mu_mat);
    est.assoc{k} = struct('id', assoc_ids, 'xyz', assoc_xyz, 'tid', assoc_tid, ...
        ...
        'quality_score', assoc_quality_score, ...
        'quality_margin', assoc_quality_margin, ...
        'robust_inflation', assoc_robust_inflation, ...
...
delete_stats.n_total = delete_stats.n_timeout_tentative + delete_stats.n_timeout_confirmed + ...
    ...
    delete_stats.n_tentative_quality + delete_stats.n_dedup_merged + ...
    delete_stats.n_capacity;
est.birth_stats = birth_stats;
est.active_assoc_stats = active_assoc_stats;
est.track_quality_stats = track_quality_stats;
...
fprintf(['  关联可信度[%s]: 观察=%d, 低可信候选边=%d, 平均=%.3f；' ...
    '主动稳健降权=%d, 自适应R低权样本[%s]=%d\n'], ...
    assoc_quality_mode, active_assoc_stats.n_quality_observed, ...
    active_assoc_stats.n_quality_low_edges, ...
    safe_ratio(active_assoc_stats.quality_sum, active_assoc_stats.n_quality_observed), ...
    active_assoc_stats.n_robust_downweighted, ...
    get_cfg_text_field(cfg, 'adapt_R_guard_mode', 'off'), ...
    active_assoc_stats.n_adapt_R_downweighted);
fprintf('  试探航迹质量[%s]: 检查=%d, 未达标=%d, 质量超时删除=%d\n', ...
    track_quality_mode, track_quality_stats.n_checks, ...
    track_quality_stats.n_not_ready, track_quality_stats.n_deleted);
fprintf('  被动角度稳健[%s]: 关联质量观察=%d, 低可信候选边=%d, 降权更新=%d\n', ...
    get_cfg_text_field(cfg, 'bearing_robust_mode', 'off'), ...
    passive_bearing_stats.n_quality_observed, ...
    passive_bearing_stats.n_quality_low_edges, ...
    passive_bearing_stats.n_robust_downweighted);
...
fprintf('航迹删除明细: 确认帧级=%d, 试探(miss=%d,NIS=%d,weight=%d,quality=%d), 去重=%d, 容量=%d\n', ...
...
fprintf('确认航迹总输出: %d 次, 成熟3D external ID: %d 个\n', ...
...
function gate = acceptance_gate_at(gates, index)
if isscalar(gates)
    gate = gates;
else
    gate = gates(index);
...
stats = struct('n_edges_over_accept_nis', 0, 'n_confirmed_primary', 0, ...
    ...
    'n_quality_low_edges', 0, 'n_quality_observed', 0, ...
    'quality_sum', 0, 'n_robust_downweighted', 0, ...
    'robust_inflation_sum', 0, 'n_adapt_R_downweighted', 0, ...
    'nis_accounting_ok', true, ...
    'secondary_pair_nis', zeros(1, 0), ...
    'secondary_pair_dist_m', zeros(1, 0));
end

function stats = empty_track_quality_stats()
stats = struct('n_checks', 0, 'n_not_ready', 0, 'n_deleted', 0, ...
    'quality_sum', 0, 'quality_hit_sum', 0);
end
...
stats = struct('n_total', 0, 'n_timeout_tentative', 0, ...
    ...
    'n_tentative_weight', 0, 'n_tentative_quality', 0, ...
    'n_dedup_merged', 0, 'n_capacity', 0);
end
...
t.L = zeros(2, 0);
t.S = zeros(0, 0);
t.Q = zeros(0, 0);        % 与S同行的关联可信度；漏检/出生种子为NaN
t.innov = {};
t.innov_weight = {};      % 自适应R样本权重（P3 guard）
t.R = {};
t.conf = zeros(1, 0);
t.miss = zeros(1, 0);
t.age  = zeros(1, 0);
...
t.quality_wait_since = zeros(1, 0);
t.N = 0;
end
...
t.Q = t.Q(:, idx);
t.innov = t.innov(idx);
t.innov_weight = t.innov_weight(idx);
t.R = t.R(idx);
t.conf = t.conf(idx);
t.miss = t.miss(idx);
t.age  = t.age(idx);
...
t.imm = t.imm(idx);
t.quality_wait_since = t.quality_wait_since(idx);
t.N = numel(t.w);
end
...
t.innov_weight = [a.innov_weight, b.innov_weight];
t.R = [a.R, b.R];
...
t.imm = [a.imm, b.imm];
t.quality_wait_since = [a.quality_wait_since, b.quality_wait_since];
t.N = a.N + b.N;
end
...
for oi = 1:N
    ...
    for oj = 1:N
        ...
        if norm(trk.m(pos_idx, j) - pc) < dist_thresh && ...
                can_merge(trk, i, j, vel_idx, vel_angle_gate, min_speed, cfg)
...
out.Q = nan(size(trk.Q, 1), n_out);
out.innov = cell(1, n_out);
out.innov_weight = cell(1, n_out);
out.R = cell(1, n_out);
...
out.quality_wait_since = nan(1, n_out);
...
for c = 1:n_out
    ...
    out.S(:, c)    = sc;
    qc = trk.Q(:, body);
    for qr = 1:size(trk.Q, 1)
        qrow = trk.Q(qr, mem);
        qrow = qrow(isfinite(qrow));
        if ~isempty(qrow)
            qc(qr) = max(qrow);
        end
    end
    ...
    out.innov_weight{c} = trk.innov_weight{body};
    out.R{c}       = trk.R{body};
    ...
    out.quality_wait_since(c) = min_finite(trk.quality_wait_since(mem));
...
function tf = can_merge(trk, i, j, vel_idx, vel_angle_gate, min_speed, cfg)
...
function tf = mode_is_on(mode)
tf = strcmpi(strtrim(char(mode)), 'on');
end

function [quality, margin] = active_association_quality(C, gates, margin_scale)
[quality, margin] = association_quality_matrix(C, gates, margin_scale);
end

function [quality, margin] = association_quality_matrix(C, gates, margin_scale)
quality = nan(size(C));
margin = nan(size(C));
if isempty(C)
    return;
...
gate_edge = reshape(max(gates(min(row, numel(gates))), eps), [], 1);
fit = max(0, min(1, 1 - cost_edge ./ gate_edge));
ambiguity = max(0, min(1, separation / margin_scale));
quality(linear) = fit .* (0.5 + 0.5 * ambiguity);
margin(linear) = separation;
end

function accept = active_quality_acceptance(quality, conf, min_tent, min_conf)
accept = false(size(quality));
for i = 1:size(quality, 1)
    if i <= numel(conf) && conf(i) == 1
        threshold = min_conf;
    else
        threshold = min_tent;
    end
    accept(i, :) = isfinite(quality(i, :)) & quality(i, :) >= threshold;
end
end

function weight = association_sample_weight(quality, robust_inflation, cfg)
floor_weight = get_cfg_field(cfg, 'adapt_R_guard_weight_floor', 0.10);
if ~isfinite(quality)
    quality = 1;
end
if ~isfinite(robust_inflation) || robust_inflation < 1
    robust_inflation = 1;
end
weight = max(floor_weight, min(1, quality / sqrt(robust_inflation)));
end

function stats = accumulate_active_quality_stats(stats, quality, robust_inflation, sample_weight)
if isfinite(quality)
    stats.n_quality_observed = stats.n_quality_observed + 1;
    stats.quality_sum = stats.quality_sum + quality;
end
if isfinite(robust_inflation) && robust_inflation > 1 + 1e-12
    stats.n_robust_downweighted = stats.n_robust_downweighted + 1;
    stats.robust_inflation_sum = stats.robust_inflation_sum + robust_inflation;
end
if isfinite(sample_weight) && sample_weight < 1 - 1e-12
    stats.n_adapt_R_downweighted = stats.n_adapt_R_downweighted + 1;
end
end

function [ready, mean_quality, n_quality] = track_quality_status(Q, S, min_hits, min_mean)
hit_mask = S == 1 | S == 4;
values = Q(hit_mask & isfinite(Q));
n_quality = numel(values);
if isempty(values)
    mean_quality = NaN;
else
    mean_quality = mean(values);
end
ready = n_quality >= min_hits && isfinite(mean_quality) && mean_quality >= min_mean;
end
...
function [R_robust, max_inflation] = bearing_robust_covariance(R, nu, S, cfg)
k = get_cfg_field(cfg, 'bearing_robust_k', 2.5);
max_allowed = get_cfg_field(cfg, 'bearing_robust_R_max_infl', 4.0);
R_robust = R;
max_inflation = 1;
sd = sqrt(max(diag(S), realmin));
for d = 1:min(numel(nu), size(R, 1))
    standardized = abs(nu(d)) / sd(d);
    if standardized > k
        inflation = min((standardized / k)^2, max_allowed);
        R_robust(d, d) = R(d, d) * inflation;
        max_inflation = max(max_inflation, inflation);
    end
end
R_robust = make_spd(R_robust);
end

function quality = passive_quality_for_ids(detail, ids)
quality = nan(size(ids));
for i = 1:numel(ids)
    mask = detail.used_mask & detail.track_id == ids(i) & ...
        isfinite(detail.quality_score);
    if any(mask)
        quality(i) = mean(detail.quality_score(mask));
    end
end
end
...
if cfg.adapt_R_enabled && size(pred.innov{ti}, 2) >= cfg.adapt_R_min_samples
    buf0 = pred.innov{ti};
    guard_on = mode_is_on(get_cfg_text_field(cfg, 'adapt_R_guard_mode', 'off'));
    if guard_on && ti <= numel(pred.innov_weight) && ...
            numel(pred.innov_weight{ti}) == size(buf0, 2)
        sample_weight = pred.innov_weight{ti}(:).';
        sample_weight(~isfinite(sample_weight) | sample_weight <= 0) = ...
            get_cfg_field(cfg, 'adapt_R_guard_weight_floor', 0.10);
        C_hat = (buf0 .* sample_weight) * buf0' / max(sum(sample_weight), realmin);
    else
    ...
    if guard_on
        max_scale = get_cfg_field(cfg, 'adapt_R_guard_max_scale', 16);
        max_step = get_cfg_field(cfg, 'adapt_R_guard_max_step', 2.0);
        for di = 1:z_dim
            baseline = max(R_meas(di, di), R_min_diag(di));
            R_new(di, di) = min(R_new(di, di), max_scale * baseline);
        end
        R_candidate = (1 - cfg.adapt_R_alpha) * R_old + cfg.adapt_R_alpha * R_new;
        for di = 1:z_dim
            old_diag = max(R_old(di, di), R_min_diag(di));
            R_candidate(di, di) = min(max(R_candidate(di, di), ...
                old_diag / max_step), old_diag * max_step);
        end
        R_use = R_candidate;
    else
...
for mi = secondary_idx
    ...
    for ti = tier_tracks
        ...
        accept_limit = acceptance_gate_at(accept_nis, ti);
        if nis_mat(ti, pmi) > accept_limit || nis_mat(ti, mi) > accept_limit
            continue;
        end
...
d = struct('used_mask', false(1, n), 'explained_mask', false(1, n), ...
    ...
    'quality_score', nan(1, n), 'quality_margin', nan(1, n), ...
    'robust_inflation', ones(1, n), ...
...
s = struct('id', zeros(1, 0), 'streak', zeros(1, 0), ...
    'last_t', zeros(1, 0), 'quality_sum', zeros(1, 0), ...
    'quality_count', zeros(1, 0));
end

function [trk, state, n_equiv] = apply_passive_equivalent_hits( ...
        trk, state, updated_ids, quality_scores, t, active_confirm_cycle, ...
        hits_per_equiv, max_gap_s)
n_equiv = 0;
valid = isfinite(updated_ids);
updated_ids = updated_ids(valid);
quality_scores = quality_scores(valid);
[updated_ids, ia] = unique(updated_ids, 'stable');
quality_scores = quality_scores(ia);
for ui = 1:numel(updated_ids)
    id = updated_ids(ui);
    quality = quality_scores(ui);
    si = find(state.id == id, 1);
    if isempty(si)
        state.id(end + 1) = id;
        state.streak(end + 1) = 0;
        state.last_t(end + 1) = -inf;
        state.quality_sum(end + 1) = 0;
        state.quality_count(end + 1) = 0;
        si = numel(state.id);
    end
    if isfinite(state.last_t(si)) && t - state.last_t(si) <= max_gap_s
        ...
        state.streak(si) = 1;
        state.quality_sum(si) = 0;
        state.quality_count(si) = 0;
    end
    if isfinite(quality)
        state.quality_sum(si) = state.quality_sum(si) + quality;
        state.quality_count(si) = state.quality_count(si) + 1;
    end
    state.last_t(si) = t;
    ...
    state.streak(si) = 0;
    if state.quality_count(si) > 0
        equivalent_quality = state.quality_sum(si) / state.quality_count(si);
    else
    ...
    % 等效确认命中：参与二维/三维M/N起始，但仍与主动距离命中S=1严格区分。
    trk.S(end, ti) = 4;
    trk.Q(end, ti) = equivalent_quality;
    n_equiv = n_equiv + 1;
end
end
...
for si = 1:numel(sources)
    ...
    assignment_cost = cost_mat;
    aux_unmatched = gate;
    [quality_mat, quality_margin_mat] = association_quality_matrix( ...
        assignment_cost, aux_unmatched * ones(1, trk.N), ...
        get_cfg_field(cfg, 'assoc_quality_margin_nis_angle', 1.0));
    quality_accept = active_quality_acceptance(quality_mat, trk.conf, ...
        get_cfg_field(cfg, 'assoc_quality_min_tentative_angle', 0.20), ...
        get_cfg_field(cfg, 'assoc_quality_min_confirmed', 0.05));
    stats.n_quality_low_edges = stats.n_quality_low_edges + ...
        nnz(isfinite(assignment_cost) & ~quality_accept);
    if mode_is_on(get_cfg_text_field(cfg, 'assoc_quality_mode', 'off'))
        assignment_cost(~quality_accept) = inf;
    end

    groups = cell(1, trk.N);
    pairs = solve_assignment(assignment_cost, aux_unmatched);
    ...
    for ti = 1:trk.N
        ...
        quality_score = quality_mat(ti, local_idx);
        quality_margin = quality_margin_mat(ti, local_idx);
        [z_pred_raw, Pzz_raw, ~, ok_raw] = ckf_bearing_stats( ...
            trk.m(:, ti), trk.P(:, :, ti), t_k, platform, cfg, pos_idx, geom);
        if ~ok_raw
            stats.n_update_rejected = stats.n_update_rejected + numel(global_idx);
            continue;
        end
        nu_raw = bearing_innovation(z_group, z_pred_raw);
        S_raw = make_spd(Pzz_raw + Rb);
        nis_raw = nu_raw' * (S_raw \ nu_raw);
        [Rb_robust, robust_inflation] = bearing_robust_covariance( ...
            Rb, nu_raw, S_raw, cfg);
        if mode_is_on(get_cfg_text_field(cfg, 'bearing_robust_mode', 'off'))
            Rb_update = Rb_robust;
        else
            Rb_update = Rb;
        end
        [~, x_upd, P_upd, ~, ~, z_pred] = ckf_update_bearing( ...
            z_group, Rb_update, trk.m(:, ti), trk.P(:, :, ti), ...
            t_k, platform, cfg, pos_idx, geom);
        ...
        detail.quality_score(global_idx) = quality_score;
        detail.quality_margin(global_idx) = quality_margin;
        detail.robust_inflation(global_idx) = robust_inflation;
        ...
        elseif trk.S(end, ti) ~= 1
            trk.S(end, ti) = 3; % 原始bearing命中；达到联合折算条件后会改写为S=4
        end
        if nis > nis_gate
            trk.nisbad(ti) = trk.nisbad(ti) + 1;
        else
        ...
        stats.n_quality_observed = stats.n_quality_observed + double(isfinite(quality_score));
        if isfinite(quality_score)
            stats.quality_sum = stats.quality_sum + quality_score;
        end
        if robust_inflation > 1 + 1e-12
            stats.n_robust_downweighted = stats.n_robust_downweighted + 1;
            stats.robust_inflation_sum = stats.robust_inflation_sum + robust_inflation;
        end
    end
...
if trk.N > 0
    ...
    event.external_3d.quality_ready = true(1, numel(idx_external));
    ...
    quality_mode = get_cfg_text_field(cfg, 'track_quality_mode', 'off');
    quality_min_hits = round(get_cfg_field(cfg, 'track_quality_upgrade_min_hits', 1));
    quality_min_mean = get_cfg_field(cfg, 'track_quality_upgrade_min_mean', 0.30);
    rate_dt = 0.05;
    ...
    for q = 1:numel(idx_external)
        ...
        q_start = max(1, size(trk.Q, 1) - upgrade_N + 1);
        [q_ready, ~, ~] = track_quality_status(trk.Q(q_start:end, ti), ...
            trk.S(q_start:end, ti), quality_min_hits, quality_min_mean);
        if mode_is_on(quality_mode)
            event.external_3d.quality_ready(q) = q_ready;
        end
...
e = struct('cycle_id', 0, 't_sec', NaN, 't_start', NaN, 't_end', NaN, ...
    ...
    'dimension_ready', false(1, 0), 'quality_ready', true(1, 0), ...
    'fresh', false(1, 0), ...
...
function M = online_matrix(M, rows, cols, fallback)
