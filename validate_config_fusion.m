...
cfg = set_default(cfg, 'joint_2d_id_offset', 1000);
cfg = set_default(cfg, 'joint_internal_2d_id_offset', 1000000);
cfg = set_default(cfg, 'result_save_mode', 'review');
cfg.result_save_mode = validate_choice(cfg.result_save_mode, ...
    {'summary', 'review', 'full'}, 'cfg.result_save_mode');
cfg = set_default(cfg, 'plot_max_points_per_track', 2000);
cfg = set_default(cfg, 'plot_max_raw_scatter_points', 100000);
cfg = set_default(cfg, 'plot_save_formats', {'png'});
cfg = set_default(cfg, 'plot_png_resolution', 150);
cfg = set_default(cfg, 'plot_close_after_save', false);
cfg = set_default(cfg, 'plot_azimuth_center_deg', 0);
cfg = set_default(cfg, 'result_review_mat_version', 'v7');
cfg = set_default(cfg, 'measurement_assignment_export_enabled', true);
cfg = set_default(cfg, 'measurement_assignment_export_file', ...
    fullfile('track_reports', 'measurement_track_assignment.xlsx'));
cfg = set_default(cfg, 'dimension_transition_export_enabled', true);
cfg = set_default(cfg, 'dimension_transition_export_file', ...
    fullfile('track_reports', 'track_dimension_transitions.xlsx'));
cfg.result_review_mat_version = validate_choice( ...
    cfg.result_review_mat_version, {'v7', 'v7.3'}, ...
    'cfg.result_review_mat_version');
if ~isscalar(cfg.plot_azimuth_center_deg) || ...
        ~isfinite(cfg.plot_azimuth_center_deg) || ...
        ~any(abs(cfg.plot_azimuth_center_deg - [0, 180]) <= 1e-9)
    error('cfg.plot_azimuth_center_deg 必须为0或180');
end
...
if ~isfield(cfg.platform, 'alt_valid_col'), cfg.platform.alt_valid_col = []; end
validate_optional_scalar_col(cfg.platform.alt_valid_col, ...
    'cfg.platform.alt_valid_col');

cfg = set_default(cfg, 'platform_max_extrapolation_s', 0.1);
cfg = set_default(cfg, 'platform_alt_max_interp_gap_s', inf);
must_be_nonnegative(cfg.platform_max_extrapolation_s, ...
    'cfg.platform_max_extrapolation_s');
must_be_nonnegative_or_inf(cfg.platform_alt_max_interp_gap_s, ...
    'cfg.platform_alt_max_interp_gap_s');
...
cfg = set_default(cfg, 'condense_spatial_index_enabled', true);
cfg = set_default(cfg, 'condense_direct_pair_limit', 50000);
cfg = set_default(cfg, 'condense_progress_interval_frames', 100);
cfg = set_default(cfg, 'assoc_quality_mode', 'off');
cfg = set_default(cfg, 'track_quality_mode', 'off');
cfg = set_default(cfg, 'bearing_robust_mode', 'off');
cfg = set_default(cfg, 'adapt_R_guard_mode', 'off');
cfg = set_default(cfg, 'assoc_quality_margin_nis_active', 2.0);
cfg = set_default(cfg, 'assoc_quality_margin_nis_angle', 1.0);
cfg = set_default(cfg, 'assoc_quality_min_tentative_active', 0.20);
cfg = set_default(cfg, 'assoc_quality_min_tentative_angle', 0.20);
cfg = set_default(cfg, 'assoc_quality_min_confirmed', 0.05);
cfg = set_default(cfg, 'track_quality_min_mean', 0.35);
cfg = set_default(cfg, 'track_quality_min_followup_hits', 2);
cfg = set_default(cfg, 'track_quality_upgrade_min_mean', 0.30);
cfg = set_default(cfg, 'track_quality_upgrade_min_hits', 1);
cfg = set_default(cfg, 'track_quality_max_wait_s', 1.0);
cfg = set_default(cfg, 'bearing_robust_k', 2.5);
cfg = set_default(cfg, 'bearing_robust_R_max_infl', 4.0);
cfg = set_default(cfg, 'adapt_R_guard_weight_floor', 0.10);
cfg = set_default(cfg, 'adapt_R_guard_max_scale', 16);
cfg = set_default(cfg, 'adapt_R_guard_max_step', 2.0);
...
cfg = set_default(cfg, 'joint_los_trend_enabled', true);
cfg = set_default(cfg, 'joint_los_trend_window', 50);
cfg = set_default(cfg, 'joint_los_trend_span_s', 1.0);
cfg = set_default(cfg, 'joint_los_trend_min_n', 10);
cfg = set_default(cfg, 'joint_los_trend_blend', 0.55);
cfg = set_default(cfg, 'joint_los_trend_min_rate_dps', 0);
cfg = set_default(cfg, 'joint_los_trend_conf_only', true);
cfg = set_default(cfg, 'joint_2d_assoc_az_weight', 4.0);
cfg = set_default(cfg, 'joint_2d_assoc_el_weight', 1.0);
cfg = set_default(cfg, 'joint_2d_rate_cost_enabled', true);
cfg = set_default(cfg, 'joint_2d_rate_cost_weight', 0.50);
cfg = set_default(cfg, 'joint_2d_rate_sigma_dps', 0.50);
cfg = set_default(cfg, 'joint_2d_rate_cost_cap', 9.0);
cfg = set_default(cfg, 'joint_2d_rate_min_dt_s', 0.02);
cfg = set_default(cfg, 'joint_2d_rate_min_history', 3);
cfg = set_default(cfg, 'joint_2d_stable_lock_enabled', true);
cfg = set_default(cfg, 'joint_2d_lock_margin', 1.0);
cfg = set_default(cfg, 'joint_2d_lock_max_az_residual_deg', 0.50);
cfg = set_default(cfg, 'joint_2d_lock_max_rate_cost', 4.0);
cfg = set_default(cfg, 'joint_2d_ambiguity_hold_enabled', true);
cfg = set_default(cfg, 'joint_2d_ambiguity_margin', 0.35);
cfg = set_default(cfg, 'joint_2d_ambiguity_max_events', 2);
...
cfg.joint_los_trend_enabled = validate_flag(cfg.joint_los_trend_enabled, ...
    'cfg.joint_los_trend_enabled');
cfg.joint_los_trend_conf_only = validate_flag(cfg.joint_los_trend_conf_only, ...
    'cfg.joint_los_trend_conf_only');
cfg.joint_2d_rate_cost_enabled = validate_flag( ...
    cfg.joint_2d_rate_cost_enabled, 'cfg.joint_2d_rate_cost_enabled');
cfg.joint_2d_stable_lock_enabled = validate_flag( ...
    cfg.joint_2d_stable_lock_enabled, 'cfg.joint_2d_stable_lock_enabled');
cfg.joint_2d_ambiguity_hold_enabled = validate_flag( ...
    cfg.joint_2d_ambiguity_hold_enabled, 'cfg.joint_2d_ambiguity_hold_enabled');
...
cfg.measurement_assignment_export_enabled = validate_flag( ...
    cfg.measurement_assignment_export_enabled, ...
    'cfg.measurement_assignment_export_enabled');
if isa(cfg.measurement_assignment_export_file, 'string')
    if ~isscalar(cfg.measurement_assignment_export_file)
        error('cfg.measurement_assignment_export_file 必须是单一路径');
    end
    cfg.measurement_assignment_export_file = ...
        char(cfg.measurement_assignment_export_file);
end
if cfg.measurement_assignment_export_enabled && ...
        (~ischar(cfg.measurement_assignment_export_file) || ...
        isempty(strtrim(cfg.measurement_assignment_export_file)))
    error('启用量测对应表导出时，cfg.measurement_assignment_export_file 必须是非空路径');
end
if cfg.measurement_assignment_export_enabled
    [~, ~, assignment_extension] = ...
        fileparts(cfg.measurement_assignment_export_file);
    if ~strcmpi(assignment_extension, '.xlsx')
        error('cfg.measurement_assignment_export_file 必须使用.xlsx扩展名');
    end
end
cfg.dimension_transition_export_enabled = validate_flag( ...
    cfg.dimension_transition_export_enabled, ...
    'cfg.dimension_transition_export_enabled');
if isa(cfg.dimension_transition_export_file, 'string')
    if ~isscalar(cfg.dimension_transition_export_file)
        error('cfg.dimension_transition_export_file 必须是单一路径');
    end
    cfg.dimension_transition_export_file = ...
        char(cfg.dimension_transition_export_file);
end
if cfg.dimension_transition_export_enabled && ...
        (~ischar(cfg.dimension_transition_export_file) || ...
        isempty(strtrim(cfg.dimension_transition_export_file)))
    error(['启用升降维事务表导出时，cfg.dimension_transition_export_file ' ...
        '必须是非空路径']);
end
if cfg.dimension_transition_export_enabled
    [~, ~, transition_extension] = ...
        fileparts(cfg.dimension_transition_export_file);
    if ~strcmpi(transition_extension, '.xlsx')
        error('cfg.dimension_transition_export_file 必须使用.xlsx扩展名');
    end
end
...
must_be_positive_integer(cfg.plot_max_points_per_track, ...
    'cfg.plot_max_points_per_track');
must_be_positive_integer(cfg.plot_max_raw_scatter_points, ...
    'cfg.plot_max_raw_scatter_points');
must_be_positive_integer(cfg.plot_png_resolution, 'cfg.plot_png_resolution');
cfg.plot_close_after_save = validate_flag(cfg.plot_close_after_save, ...
    'cfg.plot_close_after_save');
cfg.plot_save_formats = validate_plot_formats(cfg.plot_save_formats);
...
must_be_positive_integer(cfg.joint_los_trend_window, ...
    'cfg.joint_los_trend_window');
must_be_positive_integer(cfg.joint_los_trend_min_n, ...
    'cfg.joint_los_trend_min_n');
if cfg.joint_los_trend_min_n > cfg.joint_los_trend_window
    error('cfg.joint_los_trend_min_n 不能大于 cfg.joint_los_trend_window');
end
must_be_positive(cfg.joint_los_trend_span_s, 'cfg.joint_los_trend_span_s');
must_be_probability(cfg.joint_los_trend_blend, 'cfg.joint_los_trend_blend');
must_be_nonnegative(cfg.joint_los_trend_min_rate_dps, ...
    'cfg.joint_los_trend_min_rate_dps');
must_be_positive(cfg.joint_2d_assoc_az_weight, ...
    'cfg.joint_2d_assoc_az_weight');
must_be_positive(cfg.joint_2d_assoc_el_weight, ...
    'cfg.joint_2d_assoc_el_weight');
must_be_nonnegative(cfg.joint_2d_rate_cost_weight, ...
    'cfg.joint_2d_rate_cost_weight');
must_be_positive(cfg.joint_2d_rate_sigma_dps, ...
    'cfg.joint_2d_rate_sigma_dps');
must_be_positive(cfg.joint_2d_rate_cost_cap, ...
    'cfg.joint_2d_rate_cost_cap');
must_be_positive(cfg.joint_2d_rate_min_dt_s, ...
    'cfg.joint_2d_rate_min_dt_s');
must_be_positive_integer(cfg.joint_2d_rate_min_history, ...
    'cfg.joint_2d_rate_min_history');
must_be_nonnegative(cfg.joint_2d_lock_margin, 'cfg.joint_2d_lock_margin');
must_be_positive_or_inf(cfg.joint_2d_lock_max_az_residual_deg, ...
    'cfg.joint_2d_lock_max_az_residual_deg');
must_be_nonnegative_or_inf(cfg.joint_2d_lock_max_rate_cost, ...
    'cfg.joint_2d_lock_max_rate_cost');
must_be_nonnegative(cfg.joint_2d_ambiguity_margin, ...
    'cfg.joint_2d_ambiguity_margin');
must_be_nonnegative_integer(cfg.joint_2d_ambiguity_max_events, ...
    'cfg.joint_2d_ambiguity_max_events');
cfg.condense_spatial_index_enabled = validate_flag( ...
    cfg.condense_spatial_index_enabled, 'cfg.condense_spatial_index_enabled');
must_be_positive_integer(cfg.condense_direct_pair_limit, ...
    'cfg.condense_direct_pair_limit');
must_be_positive_integer(cfg.condense_progress_interval_frames, ...
    'cfg.condense_progress_interval_frames');
quality_modes = {'off', 'observe', 'on'};
cfg.assoc_quality_mode = validate_choice(cfg.assoc_quality_mode, ...
    quality_modes, 'cfg.assoc_quality_mode');
cfg.track_quality_mode = validate_choice(cfg.track_quality_mode, ...
    quality_modes, 'cfg.track_quality_mode');
cfg.bearing_robust_mode = validate_choice(cfg.bearing_robust_mode, ...
    quality_modes, 'cfg.bearing_robust_mode');
cfg.adapt_R_guard_mode = validate_choice(cfg.adapt_R_guard_mode, ...
    quality_modes, 'cfg.adapt_R_guard_mode');
must_be_positive(cfg.assoc_quality_margin_nis_active, ...
    'cfg.assoc_quality_margin_nis_active');
must_be_positive(cfg.assoc_quality_margin_nis_angle, ...
    'cfg.assoc_quality_margin_nis_angle');
must_be_probability(cfg.assoc_quality_min_tentative_active, ...
    'cfg.assoc_quality_min_tentative_active');
must_be_probability(cfg.assoc_quality_min_tentative_angle, ...
    'cfg.assoc_quality_min_tentative_angle');
must_be_probability(cfg.assoc_quality_min_confirmed, ...
    'cfg.assoc_quality_min_confirmed');
must_be_probability(cfg.track_quality_min_mean, ...
    'cfg.track_quality_min_mean');
must_be_positive_integer(cfg.track_quality_min_followup_hits, ...
    'cfg.track_quality_min_followup_hits');
must_be_probability(cfg.track_quality_upgrade_min_mean, ...
    'cfg.track_quality_upgrade_min_mean');
must_be_positive_integer(cfg.track_quality_upgrade_min_hits, ...
    'cfg.track_quality_upgrade_min_hits');
must_be_positive_or_inf(cfg.track_quality_max_wait_s, ...
    'cfg.track_quality_max_wait_s');
must_be_positive(cfg.bearing_robust_k, 'cfg.bearing_robust_k');
if ~isscalar(cfg.bearing_robust_R_max_infl) || ...
        ~isfinite(cfg.bearing_robust_R_max_infl) || ...
        cfg.bearing_robust_R_max_infl < 1
    error('cfg.bearing_robust_R_max_infl 必须是大于等于1的有限数');
end
must_be_probability(cfg.adapt_R_guard_weight_floor, ...
    'cfg.adapt_R_guard_weight_floor');
if cfg.adapt_R_guard_weight_floor <= 0
    error('cfg.adapt_R_guard_weight_floor 必须大于0');
end
if ~isscalar(cfg.adapt_R_guard_max_scale) || ...
        ~isfinite(cfg.adapt_R_guard_max_scale) || cfg.adapt_R_guard_max_scale < 1
    error('cfg.adapt_R_guard_max_scale 必须是大于等于1的有限数');
end
if ~isscalar(cfg.adapt_R_guard_max_step) || ...
        ~isfinite(cfg.adapt_R_guard_max_step) || cfg.adapt_R_guard_max_step < 1
    error('cfg.adapt_R_guard_max_step 必须是大于等于1的有限数');
end
must_be_positive_integer(cfg.joint_2d_id_offset, 'cfg.joint_2d_id_offset');
if cfg.joint_2d_id_offset <= cfg.max_tracks
    error(['cfg.joint_2d_id_offset 必须大于 cfg.max_tracks；该值同时是三维正式输出ID上界，' ...
        '二维正式输出从 joint_2d_id_offset+1 开始。']);
end
must_be_positive_integer(cfg.joint_internal_2d_id_offset, ...
    'cfg.joint_internal_2d_id_offset');
if cfg.joint_internal_2d_id_offset <= cfg.joint_2d_id_offset
    error(['cfg.joint_internal_2d_id_offset 必须大于公开输出ID分界；' ...
        '该参数仅用于内部logical事务，避免与成熟三维external ID碰撞。']);
end
...
if ~(cfg.joint_range_age_warn_s < cfg.joint_range_age_drop_s)
    error('可靠主动三维距离信息龄期阈值要求 warn < drop');
end
...
cfg = set_default(cfg, 'passive_truth_id_split_enabled', true);
cfg = set_default(cfg, 'passive_truth_id_split_max_gap_s', 10);
...
cfg.passive_truth_id_split_enabled = validate_flag( ...
    cfg.passive_truth_id_split_enabled, 'cfg.passive_truth_id_split_enabled');
...
if ~isscalar(cfg.passive_truth_id_split_max_gap_s) || ...
        isnan(cfg.passive_truth_id_split_max_gap_s) || ...
        cfg.passive_truth_id_split_max_gap_s < 0
    error('cfg.passive_truth_id_split_max_gap_s 必须是非负标量');
end
...
function validate_optional_scalar_col(col, label)
if isempty(col), return; end
if ~isnumeric(col) || ~isscalar(col) || ~isfinite(col) || ...
        col < 1 || floor(col) ~= col
    error('%s 必须为空或正整数列号', label);
end
end
...
function formats = validate_plot_formats(formats)
...
if ~iscell(formats) || isempty(formats)
    error('cfg.plot_save_formats 必须是由 png/fig 组成的非空cell数组');
end
for k = 1:numel(formats)
    if isa(formats{k}, 'string'), formats{k} = char(formats{k}); end
    if ~ischar(formats{k}) || ~any(strcmpi(formats{k}, {'png', 'fig'}))
        error('cfg.plot_save_formats 仅支持 png 和 fig');
    end
    formats{k} = lower(formats{k});
end
...
function must_be_nonnegative_integer(value, label)
if ~isscalar(value) || ~isfinite(value) || value < 0 || floor(value) ~= value
    error('%s 必须是非负整数', label);
end
end
