...
cfg.data_dir = 'C:\Users\un\Desktop\数据仿真\sim_targeted_assoc_100hz_v1_32t_150s\fusion_wide_input'; % 输入目录
cfg.active_files = {
%                     'meas_sensor1.txt'
%                     'meas_sensor2.txt'
%                     'meas_sensor3.txt'
...
cfg.read_percent = 10;                   % read_end_percent 为空时的读取比例[%]
...
cfg.active.valid_cols = [12, 13, 14];     % 首目标有效标志列；支持1/0或有效/无效
...
cfg.passive.valid_cols = [33, 34];        % 首目标有效标志列；支持1/0或有效/无效
...
cfg.platform.alt_valid_col = 45;          % 海拔高度有效性列(AS列)；[]=旧文件无此列
cfg.platform.angle_unit = 'mrad';         % 经纬度单位
cfg.platform.time_format = 'hms';         % 输入时间格式
cfg.platform_max_extrapolation_s = 1;   % 平台端点线性外推上限[s]，另限一个采样周期
cfg.platform_alt_max_interp_gap_s = inf;  % 高度无效段允许插值的最大相邻有效样本跨度[s]
...
cfg.condense_spatial_index_enabled = true; % 密集量测用空间索引排除不可能匹配的点对；不改变凝聚门限
cfg.condense_direct_pair_limit = 50000;    % 候选点对不超过此值时直接比较，避免小数据索引开销
cfg.condense_progress_interval_frames = 100; % 密集数据凝聚进度输出间隔[帧]

%% 5. [联合] 扫描事件、异步量测与重叠文件去重
cfg.async_microbatch_dt_s = 0.001;        % 实时事件微批窗口[s]；不凝聚原始角度点
...
cfg.sigma_passive_az_deg = 0.1;          % 被动方位标准差[deg]
cfg.sigma_passive_el_deg = 0.05;          % 被动俯仰标准差[deg]
...
cfg.sigma_a_cs = 5;                     % 当前统计模型加速度噪声
cfg.alpha_cs = 0.4;                      % 当前统计模型机动频率
cfg.a_max_cs = 20;                      % 当前统计模型最大加速度[m/s^2]
...
cfg.birth_guard_m = 1;                % 主动新生空间抑制半径[m]
cfg.birth_suppress_gated = true;         % 门内未分配量测禁止新生

cfg.merge_pos_dist_m = 100;              % 三维重复航迹位置合并门[m]
...
%% 8. [三维] 被动角度辅助更新
% 联合模式允许同步/纯被动事件更新已有三维航迹。单次bearing更新先记录为
% 原始角度命中；满足第9节的连续命中折算条件后，可作为一次等效确认命中，
% 与主动命中共同参与三维航迹起始确认。
...
cfg.passive_bearing_confirm_hit = false; % 单次原始bearing命中不直接写成主动确认命中；联合层另行折算
...
%% 9. [二维] 在线残余纯角度分支与跨维起始
% 三维主干未解释的角度量测在同一因果循环内进入本分支。二维、三维航迹
% 起始均可利用主动和被动证据；下列连续命中参数也供三维主干的bearing
% 等效确认命中折算使用。
...
cfg.joint_passive_confirm_consecutive_hits = 1; % 连续bearing命中折算一次等效确认命中（二维/三维共用）
cfg.joint_passive_confirm_max_gap_s = 0.20; % 连续被动命中最大间隔[s]
cfg.joint_passive_confirm_window_s = 1.50; % 二维起始确认总时间窗[s]

cfg.joint_gate_2d = 100;              % 二维角度关联卡方门
cfg.joint_angle_assoc_cov_penalty = 2.0; % 关联代价中的预测协方差惩罚
cfg.joint_unmatched_cost = 150;           % 未匹配虚拟分配代价
cfg.joint_birth_explain_nis = 1.5;       % 已有航迹对新生量测的解释门

% 联合NIS只负责二维统计门控；以下参数只改变纯二维候选间的分配排序，
% 不修改量测R、卡尔曼增益或三维航迹的角度投影关联。
cfg.joint_2d_assoc_az_weight = 9.0;      % 方位排序权重（内部按两维权重和归一化）
cfg.joint_2d_assoc_el_weight = 1.0;      % 俯仰排序权重

cfg.joint_2d_rate_cost_enabled = true;   % 视线角速度一致性参与纯二维竞争排序
cfg.joint_2d_rate_cost_weight = 0.50;    % 角速度一致性代价权重
cfg.joint_2d_rate_sigma_dps = 0.50;      % 角速度残差归一化尺度[deg/s]
cfg.joint_2d_rate_cost_cap = 9.0;        % 单条边角速度代价上限
cfg.joint_2d_rate_min_dt_s = 2;       % 候选隐含角速度最小时间间隔[s]
cfg.joint_2d_rate_min_history = 50;       % 启用趋势一致性所需最少历史点数

% 已确认纯二维航迹先锁定高可信双向最佳配对，再对剩余集合执行匈牙利。
% 竞争裕量不足时允许短时滑行，避免为瞬时全局最优交换目标身份。
cfg.joint_2d_stable_lock_enabled = false;
cfg.joint_2d_lock_margin = 1.0;          % 行/列第二候选最小排序代价裕量
cfg.joint_2d_lock_max_az_residual_deg = 0.50;
cfg.joint_2d_lock_max_rate_cost = 4.0;
%竞争暂缓关联输出
cfg.joint_2d_ambiguity_hold_enabled = false;
cfg.joint_2d_ambiguity_margin = 0.35;    % 小于该行/列竞争裕量时暂缓更新
cfg.joint_2d_ambiguity_max_events = 2;   % 连续最多暂缓事件数，之后恢复正常分配

cfg.joint_2d_accept_nis = 100;            % 宽门候选的最终二维NIS验收门
cfg.joint_2d_max_direct_gap_s = 2.0;      % 旧二维航迹允许直接续接的最大静默[s]
cfg.joint_2d_max_az_residual_deg = 6;    % 直接续接最大方位残差[deg]
cfg.joint_2d_max_el_residual_deg = 2;     % 直接续接最大俯仰残差[deg]
cfg.joint_2d_max_los_residual_deg = 10;   % 直接续接最大视线夹角[deg]

cfg.joint_tentative_timeout_s = 2;       % 二维暂态静默超时[s]
cfg.joint_confirmed_timeout_s = 4;      % 二维确认航迹静默超时[s]
cfg.joint_2d_output_max_silence_s = 0.1; % 二维正式输出最大静默[s]，不删除内部航迹
cfg.joint_2d_id_offset = 1000;        % 输出ID分界：3D从1递增，2D从1001递增
cfg.joint_internal_2d_id_offset = 1000000; % 内部logical事务命名空间，不对用户展示
cfg.joint_max_predict_dt_s = 2;          % 二维单次预测最大间隔[s]

cfg.joint_projection_cache_enabled = true; % 缓存事件内角度投影
cfg.joint_progress_interval_events = 100; % 进度日志事件间隔

% 9.2 二维角度IMM
cfg.joint_angle_q_cv = 0.5;             % 角度CV过程噪声强度
cfg.joint_angle_q_ca = 0.5;             % 角度CA过程噪声强度
cfg.joint_angle_rate_birth_std_dps = 0.5; % 新生角速度标准差[deg/s]
cfg.joint_angle_acc_birth_std_dps2 = 0.2;  % 新生角加速度标准差[deg/s^2]
cfg.joint_imm_cv_stay = 0.96;            % 统一滤波器CV保持概率
cfg.joint_imm_ca_stay = 0.94;            % 统一滤波器CA保持概率
cfg.joint_imm_cv_probability = 0.65;      % 新生CV初始概率

% 9.2.1 被动视线角速度滑窗趋势。只修正二维角度分支中的az/el角速度，
% 不改三维CKF主干状态和二维IMM模型结构。
cfg.joint_los_trend_enabled = true;       % 使用被动角度历史拟合视线角速度
cfg.joint_los_trend_window = 50;          % 每条航迹最多保留的角度样本数
cfg.joint_los_trend_span_s = 1.0;         % 施加趋势所需最小时间跨度[s]
cfg.joint_los_trend_min_n = 10;           % 施加趋势所需最少样本数
cfg.joint_los_trend_blend = 0.55;         % 趋势角速度与IMM角速度融合比例
cfg.joint_los_trend_min_rate_dps = 0.0;   % 低于该总角速度不施加趋势[deg/s]
cfg.joint_los_trend_conf_only = true;     % 仅修正确认航迹
...
% 三维正式输出的降维申请只由两个物理量控制：
%   1) 径向距离1-sigma不确定度 sigma_r；
%   2) 最近一次可靠主动三维距离更新距当前时刻的龄期。
% 预警采用 OR，正式降维申请采用 AND；实际切换仍需二维分支 ready
% 且二维/三维角度投影一致，保持事务式 pending/commit 语义。
cfg.joint_radial_sigma_warn_m = 6000;    % 径向距离1-sigma不确定度预警门[m]
cfg.joint_radial_sigma_drop_m = 12000;   % 径向距离1-sigma不确定度降维门[m]
cfg.joint_range_age_warn_s = 5;          % 可靠主动三维距离信息龄期预警门[s]
cfg.joint_range_age_drop_s = 15;         % 可靠主动三维距离信息龄期降维门[s]
...
% 10.1 关联证据、试探航迹质量与异常量测保护
% 四个模块均支持 off/observe/on：observe只记录诊断，不改变关联、更新或确认。
% 本节不包含pre-birth候选池，不在建立试探航迹前增加等待；原有M/N仍是主确认条件。
% track_quality_mode='on' 只会在M/N已满足但后续证据质量不足时暂缓正式确认。
cfg.assoc_quality_mode = 'observe';       % 关联可信度：拟合程度+同量测竞争裕量
cfg.assoc_quality_margin_nis_active = 2.0;% 主动三维关联竞争裕量归一化尺度
cfg.assoc_quality_margin_nis_angle = 1.0; % 二维角度关联竞争裕量归一化尺度
cfg.assoc_quality_min_tentative_active = 0.20; % on时三维试探航迹最小关联可信度
cfg.assoc_quality_min_tentative_angle = 0.20;  % on时二维试探航迹最小关联可信度
cfg.assoc_quality_min_confirmed = 0.05;   % on时已确认航迹最小关联可信度

cfg.track_quality_mode = 'observe';       % M/N之外的试探航迹证据质量检查
cfg.track_quality_min_mean = 0.35;        % 正式确认所需的后续命中平均质量
cfg.track_quality_min_followup_hits = 2;  % 出生点之外至少需要的质量样本数
cfg.track_quality_upgrade_min_mean = 0.30;% 二维快速升三维分支最小证据质量
cfg.track_quality_upgrade_min_hits = 1;   % 2/3升级窗内至少需要的后续质量样本数
cfg.track_quality_max_wait_s = 1.0;       % M/N已满足但质量不足的最长等待时间[s]

cfg.bearing_robust_mode = 'observe';      % 所有二维角度更新的有界协方差降权
cfg.bearing_robust_k = 2.5;               % 角度归一化残差开始降权的阈值
cfg.bearing_robust_R_max_infl = 4.0;      % 角度量测协方差最大整体膨胀倍数

cfg.adapt_R_guard_mode = 'observe';       % 主动自适应R的质量加权与上下界保护
cfg.adapt_R_guard_weight_floor = 0.10;    % 低质量但已接受样本的最小学习权重
cfg.adapt_R_guard_max_scale = 16;         % 自适应R相对传感器量测R的最大倍数
cfg.adapt_R_guard_max_step = 2.0;         % 单次自适应更新各轴最大变化倍数

%% 11. [评价] 二维、三维与总体指标
% joint_2d3d 使用同一评价骨架分别统计二维、三维和总体结果：
%   分域共用：量测关联利用率、量测正确率、已关联一致率、航迹级正确率、
%   起始延迟、角度RMSE。
%   二维：仅方位/俯仰/LOS角度误差，不计算ENU或三维位置RMSE。
%   三维：除上述共用项外，计算E/N/U及三维位置RMSE。
%   总体：严格加总二维/三维分域计数，不跨维重新匹配，也不作为物理目标数。
% 二维量测评价统一统计物理被动AE和主动AE-only；主动AE-only即使后续
% 参与二维升三维，量测侧仍只按二维输入评价，不进入三维距离/位置评价。
...
cfg.truth_id_split_enabled = false;       % 仅对主动真值按空间/时间断裂拆分
cfg.truth_id_split_dist_m = 8000;       % 主动同编号真值实例距离断裂门[m]
cfg.truth_id_split_max_gap_s = 10;       % 主动同编号真值实例时间断裂门[s]
cfg.passive_truth_id_split_enabled = true; % 纯被动真值是否按相邻AE量测时间断裂拆分
cfg.passive_truth_id_split_max_gap_s = 10; % 相邻物理被动AE时间差超过该值则拆分[s]
...
%% 12. 绘图、结果保存与离线平滑
cfg.do_plot = true;                       % 自动绘制并保存结果图
cfg.joint_plot_min_life = 3;             % 总体图最少输出点数
cfg.plot_max_points_per_track = 2000;    % 每条航迹最多绘制点数；仅显示抽样，不改滤波/评价/保存历史
cfg.plot_max_raw_scatter_points = 100000;% 每类原始量测散点最多显示点数；仅影响绘图
cfg.plot_save_dir = 'track_reports';     % 图片目录；空值=不自动保存
cfg.plot_save_formats = {'png'};         % 默认只保存PNG；需要可编辑图窗时改为{'png','fig'}
cfg.plot_png_resolution = 150;           % PNG输出分辨率[dpi]
cfg.plot_close_after_save = false;       % 保存后是否关闭本轮新建图窗
cfg.plot_azimuth_center_deg = 180;         % 0=>[-180,180)，180=>[0,360)，仅影响绘图
cfg.result_save_file = fullfile('track_reports', ...
    'fusion_result.mat');                 % MAT结果文件
cfg.result_save_mode = 'review';          % summary=统计；review=可评价/绘图/单轨复查；full=完整内核历史
cfg.result_review_mat_version = 'v7';     % review默认用紧凑MAT v7；full固定使用支持超大变量的v7.3
cfg.measurement_assignment_export_enabled = true; % 导出逐量测二维/三维航迹对应表
cfg.measurement_assignment_export_file = fullfile('track_reports', ...
    'measurement_track_assignment.xlsx'); % 七列量测-航迹对应Excel
cfg.dimension_transition_export_enabled = true; % 导出已提交的二维/三维升降维事务
cfg.dimension_transition_export_file = fullfile('track_reports', ...
    'track_dimension_transitions.xlsx');   % 升降维时间、航迹ID、判据与数值证据
