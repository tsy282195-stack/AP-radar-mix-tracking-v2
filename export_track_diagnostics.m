...
%EXPORT_TRACK_DIAGNOSTICS Export selected dimension-scoped output metrics.
% The report only prints diagnostics that are actually stored by the filter.
% result_save_mode='review' and 'full' retain association cost/NIS needed by
% this report. Summary results intentionally cannot run per-track review.
% Birth measurements do not have update residuals/NIS.
...
detailed_assoc_available = ~ismember(history_level, {'output', 'summary'});
...
fprintf(fid, '分维正式输出航迹诊断报告\n');
fprintf(fid, '生成时间\t%s\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
fprintf(fid, '航迹ID\t%.15g\n', id);
fprintf(fid, '历史级别\t%s\n', history_level);
fprintf(fid, '说明\t航迹正确判定使用一对一匹配、最少关联点和真值量测覆盖门限。\n');
fprintf(fid, '判定公式\t正确关联到匹配真值的量测数/该真值在对应评价范围的全部量测数。\n');
fprintf(fid, ['评价范围\t三维只使用RAE；二维只使用进入二维处理支路的被动AE；' ...
    '总体仅汇总两个分域结果，不跨维重新匹配。\n']);
fprintf(fid, '主评价范围\t%s\n', detail.primary_scope);
fprintf(fid, '误差参考\t使用主评价范围内、同事件同标签参考量测均值。\n');
fprintf(fid, 'RMSE_reference\tdimension_scoped_measurement_event_mean\n');
...
write_scope_summary(fid, '二维/三维分域汇总', detail.overall);
write_scope_summary(fid, '三维主动评价', detail.three_d);
write_scope_summary(fid, '二维被动AE评价', detail.two_d);

fprintf(fid, '[单轨RMSE：主评价范围标签量测参考一致性]\n');
fprintf(fid, '主评价范围\t%s\n', detail.primary_scope);
fprintf(fid, '匹配参考实例\t%.15g\n', detail.primary.matched_truth_id);
fprintf(fid, '角度样本数\t%d\n', detail.angle_rmse.n);
...
for k = 1:numel(events)
    out = event_output_history(est, k);
    for q = 1:numel(out)
        if field_num(out(q), 'id', NaN) ~= id, continue; end
        scope = scope_for_dimension(detail, field_num(out(q), 'output_dim', 0));
        reference_id = scope.matched_truth_id;
        [ref_angle, ref_position] = event_reference( ...
            est, events(k), truth_labels, k, reference_id, ...
            field_num(out(q), 'output_dim', 0));
...
fprintf(fid, ['event\ttime_s\ttype\tfilter_dim\tmeas_index\traw_id\tmapped_reference' ...
    '\tmatched_reference\tscope_applicable\tis_consistent\tmeasurement_dim\trange_updated' ...
...
for k = 1:min(numel(events), numel_field(est, 'assoc'))
    ...
    for q = find(reshape(a.id, 1, []) == id)
        ...
        filter_dim = association_filter_dim(est, k, a, q, id);
        input_dim = indexed_num(a, 'input_dim', q, filter_dim);
        primary = detail.primary;
        matched_reference = primary.matched_truth_id;
        if strcmp(detail.primary_scope, 'three_d')
            scope_applicable = measurement_dim == 3 && input_dim == 3;
        else
            scope_applicable = strncmp(type, 'passive', 7) && input_dim == 2;
        end
        is_consistent = NaN;
        if scope_applicable
            is_consistent = double(isfinite(mapped_reference) && ...
                isfinite(matched_reference) && mapped_reference == matched_reference);
        end
        ...
        fprintf(fid, ['%d\t%.9f\t%s\t%d\t%d\t%.15g\t%.15g\t%.15g\t%d\t%.0f\t%d\t%d' ...
            '\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\n'], ...
            k, meas_t, type, filter_dim, mi, raw_id, mapped_reference, ...
            matched_reference, scope_applicable, is_consistent, ...
...
primary = d.primary;
fprintf(['[单轨报告] Track %.15g: %s, 输出=%d点/%d事件, 参考实例=%.15g, ' ...
    '参考覆盖=%.2f%%, 输出覆盖=%.2f%%, 一致率=%.2f%%, 航迹级=%s\n'], ...
    d.track_id, mode, d.n_output_points, d.n_output_events, ...
    primary.matched_truth_id, 100*primary.coverage, ...
    100*primary.formal_output_coverage, 100*primary.association_consistency, ...
    pass_text(primary.is_correct));
if strcmp(history_level, 'output')
...
elseif isfield(est, 'output_history') && isfield(est.output_history, 'id')
    ids = reshape(est.output_history.id, 1, []);
end
ids = unique(ids(isfinite(ids)));
end

function out = event_output_history(est, k)
out = repmat(struct(), 0, 1);
if isfield(est, 'output') && k <= numel(est.output)
    out = est.output{k};
...
function [angle, position] = event_reference(est, e, labels, k, truth_id, dim)
angle = nan(2,1); position = nan(3,1);
...
if dim == 3
    ia = find(active_label == truth_id & has_range);
    ia = ia(ia <= size(e.active.rae,2));
    if ~isempty(ia) && size(e.active.rae,1) >= 3
        angles = e.active.rae(2:3,ia);
    end
else
    routed_2d = passive_2d_route_mask(est, k, e.passive.n_meas);
    ip = find(passive_label == truth_id & routed_2d);
    ip = ip(ip <= size(e.passive.ang,2));
    if ~isempty(ip) && size(e.passive.ang,1) >= 2
        angles = e.passive.ang(1:2,ip);
    end
end
...
if dim == 3 && ~isempty(iz) && size(e.active.xyz,1) >= 3
    Z = e.active.xyz(1:3,iz); Z = Z(:,all(isfinite(Z),1));
    if ~isempty(Z), position = mean(Z,2); end
end
end

function scope = scope_for_dimension(detail, dim)
if dim == 3
    scope = detail.three_d;
elseif dim == 2
    scope = detail.two_d;
else
    scope = detail.primary;
end
end
...
a = est.assoc{k};
for q = 1:numel(a.id)
    if ~strncmp(indexed_text(a, 'type', q, ''), 'passive', 7), continue; end
    mi = indexed_num(a, 'meas_index', q, 0);
    dim = indexed_num(a, 'input_dim', q, indexed_num(a, 'filter_dim', q, 0));
